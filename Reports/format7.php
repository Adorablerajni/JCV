...
<title>Format-7 | File Tracking & Crime Analysis Application </title>
<!-- GLOBAL STYLES -->
...
<span style="float:right;margin-right:-25px"> <input type="hidden" id="datatodisplay" name="datatodisplay" style="" /> <input type="submit" value="Export to Excel" class="btn btn-warning"/>
 ...
 <div class="row">
    <div class="col-lg-12 label-primary" style="color:#FFF">
      <form action="format7.php" method="post" name="Search" enctype="multipart/form-data" id="popup-validation" role="form">
  ...
  <?php
   ...
   {
  ?>
<p align="left" style="float:left;margin-left:-30px;">Format-7</p>
<p align="right"></p>
  <div class="mar10">
  <?php    
 ...
 ?>
    <p align="center"><span>STATEMENT OF CRPC CASES <br /> MONTH - <?php if ($_POST['datef'] == ''){	echo '';} else{echo date('m/Y', strtotime($_POST['datef']));
  ...
  ?>
    ...
    <tr>
		...
		$sec110 = '30';
		if ( !$stmt2 = $police_dsr->prepare("SELECT COUNT(id) FROM add_preventive WHERE sp_id = ? AND (istagasa_date >= ? and istagasa_date <= ?) AND preventive_type = ?") ) 
        echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
        if ( !$stmt2->bind_param("issi", $idcheck, $datef, $date1, $sec110 ) )
	   ...
	   <?php
 	   $police_dsr->select_db("ftcaaazc_dsr");
	   $sec109 = '31';
	   if ( !$stmt3 = $police_dsr->prepare("SELECT COUNT(id) FROM add_preventive WHERE sp_id = ? AND  (istagasa_date >= ? and istagasa_date <= ?) AND preventive_type = ?") ) 
       echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
       if ( !$stmt3->bind_param("issi", $idcheck, $datef, $date1, $sec109 ) )
       echo "Binding Parameter Error: ($police_dsr->errno) $police_dsr->error";
       if ( !$stmt3->execute() ) 
	...
	<?php
 	$police_dsr->select_db("ftcaaazc_dsr");
	$sec107 = '33';
	if ( !$stmt4 = $police_dsr->prepare("SELECT COUNT(id) FROM add_preventive WHERE sp_id = ? AND (istagasa_date >= ? and istagasa_date <= ?) AND preventive_type = ?") ) 
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt4->bind_param("issi", $idcheck, $datef, $date1, $sec107 ) )
    echo "Binding Parameter Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt4->execute() ) 
	...
	<?php
 	$police_dsr->select_db("ftcaaazc_dsr");
	$sec151 = '32';
	if ( !$stmt5 = $police_dsr->prepare("SELECT COUNT(id) FROM add_preventive WHERE sp_id = ? AND (istagasa_date >= ? and istagasa_date <= ?) AND preventive_type = ?") ) 
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt5->bind_param("issi", $idcheck, $datef, $date1, $sec151 ) )
    echo "Binding Parameter Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt5->execute() ) 
	...
	$nsa = '35';
	$nsa2 = '100';
	if ( !$stmt6 = $police_dsr->prepare("SELECT COUNT(id) FROM add_preventive WHERE sp_id = ? AND (istagasa_date >= ? and istagasa_date <= ?) AND (preventive_type = ? or preventive_type = ?)") ) 
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt6->bind_param("issis", $idcheck, $datef, $date1, $nsa, $nsa2 ) )
    echo "Binding Parameter Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt6->execute() ) 
	...
	<?php
 	$police_dsr->select_db("ftcaaazc_dsr");
	$externment = '34';
	if ( !$stmt7 = $police_dsr->prepare("SELECT COUNT(id) FROM add_preventive WHERE  sp_id = ? AND (istagasa_date >= ? and istagasa_date <= ?) AND preventive_type = ? ") ) 
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt7->bind_param("issi", $idcheck, $datef, $date1, $externment) )
    echo "Binding Parameter Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt7->execute() ) 
    ...
    <td>
	<?php
     $total = $cid1+$cid2+$cid3+$cid4+$cid5+$cid6;
	 $ipc6[$j]=$total;
	 echo $total;
	?>
	</td>

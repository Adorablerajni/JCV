...
<p align="right"></p>
 ...
 ?>
  ...
  <tr>
	...
	elseif ($i===13){$j ='समिति द्वारा पृथक' ; $l='s_decision'; echo "समिति द्वारा पृथक किये गये";}
	...
	<?php
        ...
        {
            $inactive = "Inactive";
        if ( !$stmt12 = $police_dsr->prepare("SELECT COUNT(DISTINCT(dsr_status.dsr_id)) FROM dsr_entries INNER JOIN dsr_status ON dsr_entries.id = dsr_status.dsr_id WHERE dsr_entries.sp_id = ? AND dsr_entries.dsr_chinhit = ? AND dsr_entries.dsr_chinhit_date LIKE ? AND dsr_status.$l = ? and dsr_entries.dsr_chinhit_date !='' and (dsr_status.status != ? or dsr_status.status is null)") ) 
        echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
        if ( !$stmt12->bind_param("issss", $branch_id, $chinhit, $like, $j, $inactive) )
        echo "Binding Parameter Error: ($police_dsr->errno) $police_dsr->error"; 
        }
         else if($i===8 || $i===9 || $i===10) 
            {
                $zero=0;
                if ( !$stmt12 = $police_dsr->prepare("SELECT COUNT(dsr_status.id) as s_id FROM dsr_status INNER JOIN dsr_entries ON dsr_entries.id = dsr_status.dsr_id WHERE dsr_entries.sp_id = ? and dsr_entries.dsr_chinhit = ? AND dsr_entries.dsr_chinhit_date LIKE ? AND dsr_status.$l = ? and dsr_entries.dsr_chinhit_date !='' and (dsr_status.criminal_id !='' or dsr_status.criminal_id != ?)"))
            echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
            if ( !$stmt12->bind_param("isssi", $branch_id, $chinhit, $like, $j, $zero) ) 
            echo "Binding Parameter Error: ($police_dsr->errno) $police_dsr->error";
            }
            else
            {
            if ( !$stmt12 = $police_dsr->prepare("SELECT COUNT(DISTINCT(dsr_status.dsr_id)) FROM dsr_entries INNER JOIN dsr_status ON dsr_entries.id = dsr_status.dsr_id WHERE dsr_entries.sp_id = ? AND dsr_entries.dsr_chinhit = ? AND dsr_entries.dsr_chinhit_date LIKE ? AND dsr_status.$l = ? and dsr_entries.dsr_chinhit_date !='' ") ) 
            echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
	...
	<?php
        ...
        ?>
            ...
            {
                if ( !$stmt12 = $police_dsr->prepare("SELECT COUNT(dsr_status.id) as s_id FROM dsr_status INNER JOIN dsr_entries ON dsr_entries.id = dsr_status.dsr_id WHERE dsr_entries.dsr_chinhit = ? AND dsr_entries.dsr_chinhit_date LIKE ? AND dsr_status.$l = ? and dsr_entries.dsr_chinhit_date !='' "))
            echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
            if ( !$stmt12->bind_param("sss", $chinhit, $like, $j) ) 
            echo "Binding Parameter Error: ($police_dsr->errno) $police_dsr->error";
            }
            else
            {
            if ( !$stmt12 = $police_dsr->prepare("SELECT COUNT(DISTINCT(dsr_status.dsr_id)) FROM dsr_entries INNER JOIN dsr_status ON dsr_entries.id = dsr_status.dsr_id WHERE dsr_entries.dsr_chinhit = ? AND dsr_entries.dsr_chinhit_date LIKE ? AND dsr_status.$l = ? and dsr_entries.dsr_chinhit_date !='' ") )  //sp_id = ? AND 
            echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";

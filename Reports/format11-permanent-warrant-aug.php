...
<p align="right"></p>
 ...
 ?>
    ...
    if ( !$stmt2->bind_param("isssss",$branch_id, $warrant_type, $datef, $datef, $datef, $sub_id) ) //, $aaropi_type
	...
	$warrant_type='स्थाई वारंट';
	$sub_id='No';
	if ( !$stmt4 = $police_summons->prepare("SELECT COUNT(id) FROM summon_list WHERE sp_id = ? AND warrant_type = ? AND tamil_date BETWEEN ? AND ? AND sub_id = ?") ) 
    echo "Prepare Error: ($police_summons->errno) $police_summons->error";
    ...
    echo "Prepare Error: ($police_summons->errno) $police_summons->error";
    if ( !$stmt5->bind_param("isssss",$branch_id, $warrant_type, $datef, $date1,$sub_id_y,$tamil_date) )
    echo "Binding Parameter Error: ($police_summons->errno) $police_summons->error";
    if ( !$stmt5->execute() ) 

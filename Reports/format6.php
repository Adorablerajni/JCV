...
<title>Format-6 | File Tracking & Crime Analysis Application </title>
<!-- GLOBAL STYLES -->
...
<span style="float:right;margin-right:-25px"> <input type="hidden" id="datatodisplay" name="datatodisplay" style="" /> <input type="submit" value="Export to Excel" class="btn btn-warning"/>
 ...
 <div class="row">
    <div class="col-lg-12 label-primary" style="color:#FFF">
      <form action="format6.php" method="post" name="Search" enctype="multipart/form-data" id="popup-validation" role="form">
  ...
  <?php
   ...
   {
  ?>
<p align="left" style="float:left;margin-left:-30px;">Format-6</p>
<p align="right"></p>
  <div class="mar10">
  <?php    
 ...
 ?>
    <p align="center"><span>MINOR ACT CASES FOR THE <br /> MONTH OF - <?php if ($_POST['datef'] == ''){	echo '';} else{echo date('m/Y', strtotime($_POST['datef']));
}?> TO <?php if ($_POST['datel'] == ''){echo '';} else{echo date('m/Y', strtotime($_POST['datel']));}?>&nbsp;&nbspINDORE ZONE</span></p>
  ...
  ?>
    ...
    <table border="1" cellspacing="0" cellpadding="5" style="text-align:center" id="print" width="960px" class="table table-striped table-bordered table-hover">
      <tr>
        ...
        <td>EXICE ACT</td>
        <td>GAMBLING ACT</td>
        <td>MOTER VEHICLE ACT</td>
        <td>E.C. ACT</td>
        <td>P.C.R. ACT</td>
        <td>RAIL ACT</td>
        <td>TELEGRAPH /MPEB ACT</td>
		<td>POLICE ACT</td>
		<td>PREV. CORRUP. ACT</td>
		<td>OTHERS</td>
		<td>TOTAL</td>
      </tr>
    ...
    <tr>
        ...
        if ( !$stmt2->bind_param("issi", $idcheck, $datef, $date1, $arms ) )
	   ...
	   <?php
 	   $police_dsr->select_db("ftcaaazc_dsr");
	   $opium = '97';
	   if ( !$stmt3 = $police_dsr->prepare("SELECT COUNT(id) FROM dsr_entries WHERE sp_id = ? AND (dsr_kaymi_date >= ? and dsr_kaymi_date <= ?) AND dsr_vidhan_minor = ?") ) 
       echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
       if ( !$stmt3->bind_param("issi", $idcheck, $datef, $date1, $opium ) )
       echo "Binding Parameter Error: ($police_dsr->errno) $police_dsr->error";
       if ( !$stmt3->execute() ) 
	...
	<?php
 	$police_dsr->select_db("ftcaaazc_dsr");
	$explosive = '26';
	if ( !$stmt4 = $police_dsr->prepare("SELECT COUNT(id) FROM dsr_entries WHERE sp_id = ? AND (dsr_kaymi_date >= ? and dsr_kaymi_date <= ?) AND dsr_vidhan_minor = ?") ) 
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt4->bind_param("issi", $idcheck, $datef, $date1, $explosive ) )
    echo "Binding Parameter Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt4->execute() ) 
	...
	<?php
 	$police_dsr->select_db("ftcaaazc_dsr");
	$exice = '20';
	if ( !$stmt5 = $police_dsr->prepare("SELECT COUNT(id) FROM dsr_entries WHERE sp_id = ? AND (dsr_kaymi_date >= ? and dsr_kaymi_date <= ?) AND dsr_vidhan_minor = ?") ) 
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt5->bind_param("issi", $idcheck, $datef, $date1, $exice ) )
    echo "Binding Parameter Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt5->execute() ) 
	...
	$gambling = '23';
    $gambling2 = '22';
	if ( !$stmt6 = $police_dsr->prepare("SELECT COUNT(id) FROM dsr_entries WHERE sp_id = ? AND (dsr_kaymi_date >= ? and dsr_kaymi_date <= ?) AND (dsr_vidhan_minor = ? OR dsr_vidhan_minor = ?)") ) 
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt6->bind_param("issii", $idcheck, $datef, $date1, $gambling, $gambling2) )
    echo "Binding Parameter Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt6->execute() ) 
	...
	<?php
 	$police_dsr->select_db("ftcaaazc_dsr");
	$motor_vehicle = '27';
	if ( !$stmt7 = $police_dsr->prepare("SELECT COUNT(id) FROM dsr_entries WHERE  sp_id = ? AND (dsr_kaymi_date >= ? and dsr_kaymi_date <= ?) AND dsr_vidhan_minor = ? ") ) 
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt7->bind_param("issi", $idcheck, $datef, $date1, $motor_vehicle) )
    echo "Binding Parameter Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt7->execute() ) 
	...
	<?php
 	$police_dsr->select_db("ftcaaazc_dsr");
	$e_c = '25';
	if ( !$stmt8 = $police_dsr->prepare("SELECT COUNT(id) FROM dsr_entries WHERE sp_id = ? AND (dsr_kaymi_date >= ? and dsr_kaymi_date <= ?) AND dsr_vidhan_minor = ?") ) 
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt8->bind_param("issi", $idcheck, $datef, $date1, $e_c ) )
    echo "Binding Parameter Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt8->execute() ) 
	...
	<?php
 	$police_dsr->select_db("ftcaaazc_dsr");
	$pcr = '98';
	if ( !$stmt9 = $police_dsr->prepare("SELECT COUNT(id) FROM dsr_entries WHERE sp_id = ? AND (dsr_kaymi_date >= ? and dsr_kaymi_date <= ?) AND dsr_vidhan_minor = ? ") ) 
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt9->bind_param("issi", $idcheck, $datef, $date1, $pcr) )
    echo "Binding Parameter Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt9->execute() ) 
	...
	<?php
 	$police_dsr->select_db("ftcaaazc_dsr");
	$rail = '91';
	if ( !$stmt10 = $police_dsr->prepare("SELECT COUNT(id) FROM dsr_entries WHERE sp_id = ? AND (dsr_kaymi_date >= ? and dsr_kaymi_date <= ?) AND dsr_vidhan_minor = ? ") ) 
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt10->bind_param("issi", $idcheck, $datef, $date1, $rail) )
    echo "Binding Parameter Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt10->execute() ) 
	...
	<?php
 	$police_dsr->select_db("ftcaaazc_dsr");
	$telegraph_mpeb = '89';
	if ( !$stmt11 = $police_dsr->prepare("SELECT COUNT(id) FROM dsr_entries WHERE sp_id = ? AND (dsr_kaymi_date >= ? and dsr_kaymi_date <= ?) AND dsr_vidhan_minor = ? ") ) 
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt11->bind_param("issi", $idcheck, $datef, $date1, $telegraph_mpeb) )
    echo "Binding Parameter Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt11->execute() ) 
	...
	<?php
 	$police_dsr->select_db("ftcaaazc_dsr");
	$police = '28';
	if ( !$stmt12= $police_dsr->prepare("SELECT COUNT(id) FROM dsr_entries WHERE sp_id = ? AND (dsr_kaymi_date >= ? and dsr_kaymi_date <= ?) AND dsr_vidhan_minor = ? ") ) 
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt12->bind_param("issi", $idcheck, $datef, $date1, $police) )
    echo "Binding Parameter Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt12->execute() ) 
	...
	<?php
 	$police_dsr->select_db("ftcaaazc_dsr");
	$pre_corrup = '84';
	if ( !$stmt13= $police_dsr->prepare("SELECT COUNT(id) FROM dsr_entries WHERE sp_id = ? AND (dsr_kaymi_date >= ? and dsr_kaymi_date <= ?) AND dsr_vidhan_minor = ? ") ) 
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt13->bind_param("issi", $idcheck, $datef, $date1, $pre_corrup) )
    echo "Binding Parameter Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt13->execute() ) 
	...
	<?php
 	$police_dsr->select_db("ftcaaazc_dsr");
	$police = '28'; $telegraph_mpeb = '89'; $rail = '91'; $pcr = '98'; $e_c = '25'; $motor_vehicle = '27'; $gambling = '23'; $exice = '20'; $explosive = '26'; $opium = '97'; $arms = '21'; $pre_corrup = '84';
	if ( !$stmt14= $police_dsr->prepare("SELECT COUNT(id) FROM dsr_entries WHERE sp_id = ? AND (dsr_kaymi_date >= ? and dsr_kaymi_date <= ?) AND (dsr_vidhan_minor != ? OR dsr_vidhan_minor != ? OR dsr_vidhan_minor != ? OR dsr_vidhan_minor != ? OR dsr_vidhan_minor != ? OR dsr_vidhan_minor != ? OR dsr_vidhan_minor != ? OR dsr_vidhan_minor != ? OR dsr_vidhan_minor != ? OR dsr_vidhan_minor != ? OR dsr_vidhan_minor != ? OR dsr_vidhan_minor != ?)") ) 
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt14->bind_param("issiiiiiiiiiiii", $idcheck, $datef, $date1,$police,$telegraph_mpeb,$rail,$pcr,$e_c,$motor_vehicle,$gambling,$exice,$explosive,$opium,$arms,$pre_corrup) )
    echo "Binding Parameter Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt14->execute() ) 
    ...
    <td>
	<?php
     $total = $cid1+$cid2+$cid3+$cid4+$cid5+$cid7+$cid8+$cid9+$cid10+$cid11+$cid12+$cid13;
	 echo $total;
	 $ipc13[$j]=$total;	
	?>
	</td>

...
<span style="float:right;margin-right:-25px"> <input type="hidden" id="datatodisplay" name="datatodisplay" style="" /> <input type="submit" value="Export to Excel"/>
  ...
  <div class="row">
    <div class="col-lg-12">
      <form action="format1-ipc.php" method="post" name="Search" enctype="multipart/form-data" id="popup-validation" role="form">
  ...
  <?php
    ...
    {
  ?>
  <p align="left" style="float:left;margin-left:-30px;">Format-1-ipc</p>
  <p align="right"></p>
  <div class="mar10">
 <?php   
...
<table border="1" cellspacing="0" cellpadding="5" style="font-size:12px;line-height:20px;margin-left:-30px;text-align:center" id="print" width="960px">
  ...
  <tr>
	...
	$dsr_balig1 = 'वयस्क';
	$dsr_gender = 'महिला';
	if ( !$stmt2 = $police_dsr->prepare("SELECT COUNT(id) FROM dsr_entries WHERE sp_id = ? AND dsr_kaymi_date BETWEEN ? AND ? AND (dsr_vidhan_ipc=? OR dsr_vidhan_ipc = ?) AND dsr_gender = ? AND dsr_balig = ? ") ) 
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt2->bind_param("issssss",$branch_id1,$datef, $date1,$j,$k,$dsr_gender,$dsr_balig1) )
	...
	if ( !$stmt2 = $police_dsr->prepare("SELECT COUNT(id) FROM dsr_entries WHERE sp_id = ? AND dsr_kaymi_date BETWEEN ? AND ? AND (dsr_vidhan_ipc=? OR dsr_vidhan_ipc = ?) AND dsr_gender = ? AND dsr_balig = ? ") ) 
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt2->bind_param("issssss",$branch_id1,$datef, $date1,$j,$k,$dsr_gender,$dsr_balig2) )
    echo "Binding Parameter Error: ($police_dsr->errno) $police_dsr->error";
		}
		elseif($i===16)
		{
	$dsr_kdnp_rsn = 'फिरौती के लिये';
	if ( !$stmt2 = $police_dsr->prepare("SELECT COUNT(id) FROM dsr_entries WHERE sp_id = ? AND dsr_kaymi_date BETWEEN ? AND ? AND (dsr_vidhan_ipc=? OR dsr_vidhan_ipc = ?) AND dsr_kdnp_rsn = ? ") ) 
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
	...
	$dsr_balig1 = 'वयस्क';
	$dsr_gender = 'महिला';
    if ( !$stmt17 = $police_dsr->prepare("SELECT COUNT(id) FROM dsr_entries WHERE sp_id = ? AND dsr_kaymi_date BETWEEN ? AND ? AND (dsr_vidhan_ipc=? OR dsr_vidhan_ipc = ?) AND dsr_gender = ? AND dsr_balig = ?") ) 
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt17->bind_param("issssss",$branch_id1,$newDate1,$newDate2,$j,$k,$dsr_gender,$dsr_balig1) )
    echo "Binding Parameter Error: ($police_dsr->errno) $police_dsr->error";
		}
		elseif($i===15)
		{
		...
		if ( !$stmt17 = $police_dsr->prepare("SELECT COUNT(id) FROM dsr_entries WHERE sp_id = ? AND dsr_kaymi_date BETWEEN ? AND ? AND (dsr_vidhan_ipc=? OR dsr_vidhan_ipc = ?) AND dsr_gender = ? AND dsr_balig = ?") ) 
        echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
        if ( !$stmt17->bind_param("issssss",$branch_id1,$newDate1,$newDate2,$j,$k,$dsr_gender,$dsr_balig2) )
        echo "Binding Parameter Error: ($police_dsr->errno) $police_dsr->error";
		}
		elseif($i===16)
		{
	$dsr_kdnp_rsn = 'फिरौती के लिये';
    if ( !$stmt17 = $police_dsr->prepare("SELECT COUNT(id) FROM dsr_entries WHERE sp_id = ? AND dsr_kaymi_date BETWEEN ? AND ? AND (dsr_vidhan_ipc=? OR dsr_vidhan_ipc = ?) AND dsr_kdnp_rsn = ? ") ) 
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
	...
	if ( !$stmt17 = $police_dsr->prepare("SELECT COUNT(id) FROM dsr_entries WHERE sp_id = ? AND dsr_kaymi_date BETWEEN ? AND ? AND dsr_vidhan_ipc=?") ) 
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
	...
	$dsr_balig1 = 'वयस्क';
	$dsr_gender = 'महिला';
    if ( !$stmt18 = $police_dsr->prepare("SELECT COUNT(id) FROM dsr_entries WHERE sp_id = ? AND dsr_kaymi_date BETWEEN ? AND ? AND (dsr_vidhan_ipc=? OR dsr_vidhan_ipc = ?) AND dsr_gender = ? AND dsr_balig = ?") ) 
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt18->bind_param("issssss",$branch_id1,$newDate3, $newDate4,$j,$k,$dsr_gender,$dsr_balig1) )
    echo "Binding Parameter Error: ($police_dsr->errno) $police_dsr->error";
		}
		elseif($i===15)
		{
	...
	if ( !$stmt18 = $police_dsr->prepare("SELECT COUNT(id) FROM dsr_entries WHERE sp_id = ? AND dsr_kaymi_date BETWEEN ? AND ? AND (dsr_vidhan_ipc=? OR dsr_vidhan_ipc = ?) AND dsr_gender = ? AND dsr_balig = ?") ) 
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt18->bind_param("issssss",$branch_id1,$newDate3, $newDate4,$j,$k,$dsr_gender,$dsr_balig2) )
    echo "Binding Parameter Error: ($police_dsr->errno) $police_dsr->error";
		}
		elseif($i===16)
		{
	$dsr_kdnp_rsn = 'फिरौती के लिये';
    if ( !$stmt18 = $police_dsr->prepare("SELECT COUNT(id) FROM dsr_entries WHERE sp_id = ? AND dsr_kaymi_date BETWEEN ? AND ? AND (dsr_vidhan_ipc=? OR dsr_vidhan_ipc = ?) AND dsr_kdnp_rsn = ?") ) 
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
	...
	if ( !$stmt18 = $police_dsr->prepare("SELECT COUNT(id) FROM dsr_entries WHERE sp_id = ? AND dsr_kaymi_date BETWEEN ? AND ? AND dsr_vidhan_ipc=?") ) 
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
    ...
    if ( !$stmt18 = $police_dsr->prepare("SELECT SUM(crime) FROM old_records WHERE district = ? AND start_date BETWEEN ? AND ? AND (dsr_vidhan_ipc=? OR dsr_vidhan_ipc = ?) AND kidnapping = ? ") ) 
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
	...
	if ( !$stmt18 = $police_dsr->prepare("SELECT SUM(crime) FROM old_records WHERE district = ? AND start_date BETWEEN ? AND ? AND dsr_vidhan_ipc=?") ) 
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt18->bind_param("isss",$branch_id1,$newDate3, $newDate4,$j) )
    echo "Binding Parameter Error: ($police_dsr->errno) $police_dsr->error";
		}
  ...
  <tr>
	...
	<td>
	 ...
	 $gum = '102';
	 //$nakbajni = '94';
	if ( !$stmt2 = $police_dsr->prepare("SELECT COUNT(id) FROM dsr_entries WHERE sp_id = ? AND dsr_kaymi_date BETWEEN ? AND ? AND dsr_vidhan_ipc != ? AND dsr_vidhan_ipc != '' AND dsr_vidhan_ipc is not NULL") ) 
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
	...
	<span><?php echo $ipc[29][$l];?></span>
	  ...
	  $gum = '102';
	  //$nakbajni = '94';
	if ( !$stmt17 = $police_dsr->prepare("SELECT COUNT(id) FROM dsr_entries WHERE sp_id = ? AND dsr_kaymi_date BETWEEN ? AND ? AND dsr_vidhan_ipc != ? AND dsr_vidhan_ipc != '' AND dsr_vidhan_ipc is not NULL") ) 
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
	...
	<?php if($newyear2=='2018')
		...
		$police_dsr->select_db("ftcaaazc_dsr");
	$gum = '102';
	if ( !$stmt18 = $police_dsr->prepare("SELECT COUNT(id) FROM dsr_entries WHERE sp_id = ? AND dsr_kaymi_date BETWEEN ? AND ? AND dsr_vidhan_ipc != ? AND dsr_vidhan_ipc != '' AND dsr_vidhan_ipc is not NULL") ) 
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
	...
	$gum = '102';
	$nakbajni = '94';
	if ( !$stmt18 = $police_dsr->prepare("SELECT SUM(crime) FROM old_records WHERE district = ? AND start_date BETWEEN ? AND ? AND dsr_vidhan_ipc != ? AND dsr_vidhan_ipc != '' AND dsr_vidhan_ipc is not NULL AND dsr_vidhan_ipc != ?") ) 
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";

...
<p align="right"></p>
  ...
  <?php   	
    $sp='SP';
    $sp_id = '6';
    $stmt1 = $police_tracking->prepare("SELECT id,branch_name,city FROM branch_tbl where designation = ? and id != ?");
    $stmt1->bind_param("si", $sp, $sp_id);
    $stmt1->execute();
    $stmt1->store_result();
 ...
 ?>
    <table border="1" cellspacing="0" cellpadding="5" style="font-size:12px;line-height:20px;margin-left:-30px;text-align:center" id="print" width="960px">
	  ...
	  ?>
            ...
            //$year18 ='2018';
            $like = "%" . $year . "%";
            if ( !$stmt2 = $police_dsr->prepare("SELECT COUNT(id) FROM dsr_entries WHERE sp_id = ? AND dsr_chinhit = ? AND dsr_chinhit_date LIKE ? ") ) 
            echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
            if ( !$stmt2->bind_param("iss", $branch_id, $chinhit, $like) )
            ...
            <?php
            $police_dsr->select_db("ftcaaazc_dsr");
            if ( !$stmt3 = $police_dsr->prepare("SELECT COUNT(id) FROM dsr_entries WHERE sp_id = ? AND dsr_chinhit_date BETWEEN ? AND ? AND dsr_chinhit = ?") ) 
            echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
            if ( !$stmt3->bind_param("isss", $branch_id, $datef, $date1, $chinhit) )
            echo "Binding Parameter Error: ($police_dsr->errno) $police_dsr->error";
            if ( !$stmt3->execute() ) 
            ...
            $nirakrat='निराकृत';
            $police_dsr->select_db("ftcaaazc_dsr");
            if ( !$stmt4 = $police_dsr->prepare("SELECT COUNT(id) FROM dsr_entries WHERE sp_id = ? AND c_status_date BETWEEN ? AND ? AND c_status = ? AND dsr_chinhit = ? AND dsr_chinhit_date is not null") ) 
            echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
            if ( !$stmt4->bind_param("issss", $branch_id, $datef, $date1, $nirakrat, $chinhit) )
            echo "Binding Parameter Error: ($police_dsr->errno) $police_dsr->error";
            if ( !$stmt4->execute() ) 
            ...
            $saja='सजा';
            $police_dsr->select_db("ftcaaazc_dsr");
            if ( !$stmt5 = $police_dsr->prepare("SELECT COUNT(id) FROM dsr_entries WHERE sp_id = ? AND c_status_date BETWEEN ? AND ? AND c_decision = ? AND dsr_chinhit = ? AND dsr_chinhit_date is not null") ) 
            echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
            if ( !$stmt5->bind_param("issss", $branch_id, $datef, $date1, $saja, $chinhit) )
            echo "Binding Parameter Error: ($police_dsr->errno) $police_dsr->error";
            if ( !$stmt5->execute() ) 
            ...
            $doshmukt='दोषमुक्त';
            $police_dsr->select_db("ftcaaazc_dsr");
            if ( !$stmt6 = $police_dsr->prepare("SELECT COUNT(id) FROM dsr_entries WHERE sp_id = ? AND c_status_date BETWEEN ? AND ? AND c_decision = ? AND dsr_chinhit = ? AND dsr_chinhit_date is not null") ) 
            echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
            if ( !$stmt6->bind_param("issss", $branch_id, $datef, $date1, $doshmukt, $chinhit) )
            echo "Binding Parameter Error: ($police_dsr->errno) $police_dsr->error";
            if ( !$stmt6->execute() ) 

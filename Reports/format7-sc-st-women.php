...
<p align="right"></p>
 ...
 <br/>
    <table border="1" cellspacing="0" cellpadding="5" style="text-align:center" id="print" width="960px" class="table table-striped table-bordered table-hover">
	 ...
	 $dsr_caste='SC';
	if ( !$stmt2 = $police_dsr->prepare("SELECT COUNT(DISTINCT(pidit_list.dsr_id)) FROM pidit_list INNER JOIN dsr_entries ON pidit_list.dsr_id = dsr_entries.id WHERE dsr_entries.sp_id = ? AND (dsr_entries.dsr_kaymi_date >= ? and dsr_entries.dsr_kaymi_date <= ?) AND pidit_list.pidit_caste = ? AND dsr_entries.atyachar_adhiniyam = ?") ) 
	...
	<span><?php echo $cid2;?></span>
	  ...
	  $police_dsr->select_db("ftcaaazc_dsr");
	  $dsr_caste='SC';
	if ( !$stmt17 = $police_dsr->prepare("SELECT COUNT(DISTINCT(pidit_list.dsr_id)) FROM pidit_list INNER JOIN dsr_entries ON pidit_list.dsr_id = dsr_entries.id WHERE dsr_entries.sp_id = ? AND (dsr_entries.dsr_kaymi_date >= ? and dsr_entries.dsr_kaymi_date <= ?) AND pidit_list.pidit_caste = ? AND dsr_entries.atyachar_adhiniyam = ?") ) 
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
	...
	<span><?php echo $cid17;?></span>
		...
		$police_dsr->select_db("ftcaaazc_dsr");
		$dsr_caste='SC';
	if ( !$stmt18 = $police_dsr->prepare("SELECT COUNT(DISTINCT(pidit_list.dsr_id)) FROM pidit_list INNER JOIN dsr_entries ON pidit_list.dsr_id = dsr_entries.id WHERE dsr_entries.sp_id = ? AND (dsr_entries.dsr_kaymi_date >= ? and dsr_entries.dsr_kaymi_date <= ?) AND pidit_list.pidit_caste = ? AND dsr_entries.atyachar_adhiniyam = ?") ) 
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
...
$ip2[$j] = 0 ;		
	...
	<span><?php// echo $cid18;?></span>
	  ...
	  $police_dsr->select_db("ftcaaazc_dsr");
	  $dsr_caste1='ST';
	if ( !$stmt3 = $police_dsr->prepare("SELECT COUNT(DISTINCT(pidit_list.dsr_id)) FROM pidit_list INNER JOIN dsr_entries ON pidit_list.dsr_id = dsr_entries.id WHERE dsr_entries.sp_id = ? AND (dsr_entries.dsr_kaymi_date >= ? and dsr_entries.dsr_kaymi_date <= ?) AND pidit_list.pidit_caste = ? AND dsr_entries.atyachar_adhiniyam = ?") ) 
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
	...
	<span><?php echo $cid3;?></span>
	  ...
	  $police_dsr->select_db("ftcaaazc_dsr");
	  $dsr_caste1='ST';
	if ( !$stmt13 = $police_dsr->prepare("SELECT COUNT(DISTINCT(pidit_list.dsr_id)) FROM pidit_list INNER JOIN dsr_entries ON pidit_list.dsr_id = dsr_entries.id WHERE dsr_entries.sp_id = ? AND (dsr_entries.dsr_kaymi_date >= ? and dsr_entries.dsr_kaymi_date <= ?) AND pidit_list.pidit_caste = ? AND dsr_entries.atyachar_adhiniyam = ?") ) 
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
	...
	<span><?php echo $cid13;?></span>
		...
		$police_dsr->select_db("ftcaaazc_dsr");
		$dsr_caste1='ST';
	if ( !$stmt8 = $police_dsr->prepare("SELECT COUNT(DISTINCT(pidit_list.dsr_id)) FROM pidit_list INNER JOIN dsr_entries ON pidit_list.dsr_id = dsr_entries.id WHERE dsr_entries.sp_id = ? AND (dsr_entries.dsr_kaymi_date >= ? and dsr_entries.dsr_kaymi_date <= ?) AND pidit_list.pidit_caste = ? AND dsr_entries.atyachar_adhiniyam = ?") ) 
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
	...
	<span><?php echo $cid8;?></span>
	  ...
	  $dsr_caste='SC';
	  $dsr_caste1='ST';
	  $dsr_vidhan_ipc_accident='14';
	if ( !$stmt4 = $police_dsr->prepare("SELECT COUNT(DISTINCT(pidit_list.dsr_id)) FROM pidit_list INNER JOIN dsr_entries ON pidit_list.dsr_id = dsr_entries.id WHERE dsr_entries.sp_id = ? AND (dsr_entries.dsr_kaymi_date >= ? and dsr_entries.dsr_kaymi_date <= ?) AND pidit_list.pidit_gender = ? AND dsr_entries.dsr_vidhan_ipc != ?") ) 
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt4->bind_param("isssi", $branch_id, $datef, $date1, $dsr_gender, $dsr_vidhan_ipc_accident) ) //,$dsr_caste,$dsr_caste1
    echo "Binding Parameter Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt4->execute() ) 
	...
	<span><?php echo $cid4;?></span>
	  ...
	  $dsr_caste1='ST';
	  $dsr_vidhan_ipc_accident='14';
	if ( !$stmt14 = $police_dsr->prepare("SELECT COUNT(DISTINCT(pidit_list.dsr_id)) FROM pidit_list INNER JOIN dsr_entries ON pidit_list.dsr_id = dsr_entries.id WHERE dsr_entries.sp_id = ? AND (dsr_entries.dsr_kaymi_date >= ? and dsr_entries.dsr_kaymi_date <= ?) AND pidit_list.pidit_gender = ? AND dsr_entries.dsr_vidhan_ipc != ?") )      //SELECT COUNT(id) FROM dsr_entries WHERE sp_id = ? AND (dsr_kaymi_date >= ? and dsr_kaymi_date <= ?) AND dsr_gender=? AND (dsr_caste != ? OR dsr_caste != ?) 
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt14->bind_param("isssi", $branch_id, $newDate1, $newDate2, $dsr_gender, $dsr_vidhan_ipc_accident) ) //,$dsr_caste,$dsr_caste1
    echo "Binding Parameter Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt14->execute() ) 
	...
	<span><?php echo $cid14;?></span>
		...
		$police_dsr->select_db("ftcaaazc_dsr");
	     ...
	     $dsr_caste1='ST';
		 $dsr_vidhan_ipc_accident='14';
	if ( !$stmt28 = $police_dsr->prepare("SELECT COUNT(DISTINCT(pidit_list.dsr_id)) FROM pidit_list INNER JOIN dsr_entries ON pidit_list.dsr_id = dsr_entries.id WHERE dsr_entries.sp_id = ? AND (dsr_entries.dsr_kaymi_date >= ? and dsr_entries.dsr_kaymi_date <= ?) AND pidit_list.pidit_gender = ? AND dsr_entries.dsr_vidhan_ipc != ?") ) 
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt28->bind_param("isssi", $branch_id, $newDate3, $newDate4, $dsr_gender, $dsr_vidhan_ipc_accident) ) //,$dsr_caste,$dsr_caste1
    echo "Binding Parameter Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt28->execute() ) 

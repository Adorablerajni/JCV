...
}?> से  <?php if ($_POST['datel'] == ''){echo '';} else{echo date('d-m-Y', strtotime($_POST['datel']));}?>तक &nbsp;&nbsp इन्दौर जोन</span></p>
 <br />
 <?php	  
  $datef = $_POST['datef'];
  $date1 = $_POST['datel'];
...
echo "0%";	
	...
	$sp='SP';
	$police_tracking->select_db("ftcaaazc_epfts");
	$stmt11 = $police_tracking->prepare("SELECT id,branch_name,city FROM branch_tbl where designation = ?");
    $stmt11->bind_param("s", $sp);
    $stmt11->execute();
    $stmt11->store_result();

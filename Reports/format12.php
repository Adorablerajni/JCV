...
<?php 
  ...
  <tr>
	...
	elseif ($i===13){ $j = 39 ; $k = 15; echo " शीलभंग (354) ";}
    ...
    if ( !$stmt2->bind_param("iiissss", $j, $k, $branch_id, $sc, $datef, $date1, $atyachar) )
	...
	if ($i===13)
	{
	if ( !$stmt3 = $police_dsr->prepare("SELECT COUNT(DISTINCT(pidit_list.dsr_id)) FROM pidit_list INNER JOIN dsr_entries ON pidit_list.dsr_id = dsr_entries.id WHERE (dsr_entries.dsr_vidhan_ipc = ? OR dsr_entries.dsr_vidhan_ipc = ?) AND dsr_entries.sp_id = ? AND pidit_list.pidit_caste = ? AND (dsr_entries.dsr_kaymi_date >= ? and dsr_entries.dsr_kaymi_date <= ?) AND dsr_entries.atyachar_adhiniyam = ?") )
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt3->bind_param("iiissss", $j, $k, $branch_id, $st, $datef, $date1, $atyachar) )
    echo "Binding Parameter Error: ($police_dsr->errno) $police_dsr->error";	
	}
	else
	{
...
$ipc1[$i]=$cid2;	
	...
	if ($i===13)
	{
	if ( !$stmt4 = $police_dsr->prepare("SELECT COUNT(DISTINCT(pidit_list.dsr_id)) FROM dsr_status INNER JOIN pidit_list ON dsr_status.dsr_id = pidit_list.dsr_id INNER JOIN dsr_entries ON pidit_list.dsr_id = dsr_entries.id WHERE (dsr_entries.dsr_vidhan_ipc = ? OR dsr_entries.dsr_vidhan_ipc = ?) AND dsr_entries.sp_id = ? AND pidit_list.pidit_caste = ? AND (dsr_entries.dsr_kaymi_date >= ? and dsr_entries.dsr_kaymi_date <= ?) AND dsr_status.s_status = ? AND dsr_entries.atyachar_adhiniyam = ? AND dsr_status.status != ?") )
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt4->bind_param("iiissssss", $j, $k, $branch_id, $sc, $datef, $date1, $vivechna, $atyachar, $inactive) )
    echo "Binding Parameter Error: ($police_dsr->errno) $police_dsr->error";	
	}
	else
	{
...
$ipc2[$i]=$cid3;	
	...
	if ($i===13)
	{
	if ( !$stmt5 = $police_dsr->prepare("SELECT COUNT(DISTINCT(pidit_list.dsr_id)) FROM dsr_status INNER JOIN pidit_list ON dsr_status.dsr_id = pidit_list.dsr_id INNER JOIN dsr_entries ON pidit_list.dsr_id = dsr_entries.id WHERE (dsr_entries.dsr_vidhan_ipc = ? OR dsr_entries.dsr_vidhan_ipc = ?) AND dsr_entries.sp_id = ? AND pidit_list.pidit_caste = ? AND (dsr_entries.dsr_kaymi_date >= ? and dsr_entries.dsr_kaymi_date <= ?) AND dsr_status.s_status = ? AND dsr_entries.atyachar_adhiniyam = ? AND dsr_status.status != ?") )
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt5->bind_param("iiissssss", $j, $k, $branch_id, $sc, $datef, $date1, $vivechna, $atyachar, $inactive) )
    echo "Binding Parameter Error: ($police_dsr->errno) $police_dsr->error";	
	}
	else
	{
...
$ipc3[$i]=$cid4;	
	...
	if ($i===13)
	{
	if ( !$stmt6 = $police_dsr->prepare("SELECT COUNT(DISTINCT(pidit_list.dsr_id)) FROM dsr_status INNER JOIN pidit_list ON dsr_status.dsr_id = pidit_list.dsr_id INNER JOIN dsr_entries ON pidit_list.dsr_id = dsr_entries.id WHERE (dsr_entries.dsr_vidhan_ipc = ? OR dsr_entries.dsr_vidhan_ipc = ?) AND dsr_entries.sp_id = ? AND pidit_list.pidit_caste = ? AND (dsr_entries.dsr_kaymi_date >= ? and dsr_entries.dsr_kaymi_date <= ?) AND dsr_status.s_status = ? AND dsr_entries.atyachar_adhiniyam = ? ") )
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt6->bind_param("iiisssss", $j, $k, $branch_id, $sc, $datef, $date1, $bandi, $atyachar) )
    echo "Binding Parameter Error: ($police_dsr->errno) $police_dsr->error";	
	}
	else
	{
	...
	if ($i===13)
	{
	if ( !$stmt7 = $police_dsr->prepare("SELECT COUNT(DISTINCT(pidit_list.dsr_id)) FROM dsr_status INNER JOIN pidit_list ON dsr_status.dsr_id = pidit_list.dsr_id INNER JOIN dsr_entries ON pidit_list.dsr_id = dsr_entries.id WHERE (dsr_entries.dsr_vidhan_ipc = ? OR dsr_entries.dsr_vidhan_ipc = ?) AND dsr_entries.sp_id = ? AND pidit_list.pidit_caste = ? AND (dsr_entries.dsr_kaymi_date >= ? and dsr_entries.dsr_kaymi_date <= ?) AND dsr_status.s_status = ? AND dsr_entries.atyachar_adhiniyam = ? ") )
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt7->bind_param("iiisssss", $j, $k, $branch_id, $sc, $datef, $date1, $bandi, $atyachar) )
    echo "Binding Parameter Error: ($police_dsr->errno) $police_dsr->error";	
	}
	else
	{
...
$ipc5[$i]=$cid6;	
	...
	if ($i===13)
	{
	if ( !$stmt8 = $police_dsr->prepare("SELECT COUNT(DISTINCT(pidit_list.dsr_id)) FROM dsr_status INNER JOIN pidit_list ON dsr_status.dsr_id = pidit_list.dsr_id INNER JOIN dsr_entries ON pidit_list.dsr_id = dsr_entries.id WHERE (dsr_entries.dsr_vidhan_ipc = ? OR dsr_entries.dsr_vidhan_ipc = ?) AND dsr_entries.sp_id = ? AND pidit_list.pidit_caste = ? AND (dsr_entries.dsr_kaymi_date >= ? and dsr_entries.dsr_kaymi_date <= ?) AND dsr_status.s_status = ? AND dsr_entries.atyachar_adhiniyam = ? ") )
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt8->bind_param("iiisssss", $j, $k, $branch_id, $sc, $datef, $date1, $khatma, $atyachar) )
    echo "Binding Parameter Error: ($police_dsr->errno) $police_dsr->error";	
	}
	else
	{
...
$ipc6[$i]=$cid7;	
	...
	if ($i===13)
	{
	if ( !$stmt9 = $police_dsr->prepare("SELECT COUNT(DISTINCT(pidit_list.dsr_id)) FROM dsr_status INNER JOIN pidit_list ON dsr_status.dsr_id = pidit_list.dsr_id INNER JOIN dsr_entries ON pidit_list.dsr_id = dsr_entries.id WHERE (dsr_entries.dsr_vidhan_ipc = ? OR dsr_entries.dsr_vidhan_ipc = ?) AND dsr_entries.sp_id = ? AND pidit_list.pidit_caste = ? AND (dsr_entries.dsr_kaymi_date >= ? and dsr_entries.dsr_kaymi_date <= ?) AND dsr_status.s_status = ? AND dsr_entries.atyachar_adhiniyam = ? ") )
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt9->bind_param("iiisssss", $j, $k, $branch_id, $sc, $datef, $date1, $khatma, $atyachar) )
    echo "Binding Parameter Error: ($police_dsr->errno) $police_dsr->error";	
	}
	else
	{
...
$ipc7[$i]=$cid8;	
	...
	if ($i===13)
	{
	if ( !$stmt10 = $police_dsr->prepare("SELECT COUNT(DISTINCT(pidit_list.dsr_id)) FROM dsr_status INNER JOIN pidit_list ON dsr_status.dsr_id = pidit_list.dsr_id INNER JOIN dsr_entries ON pidit_list.dsr_id = dsr_entries.id WHERE (dsr_entries.dsr_vidhan_ipc = ? OR dsr_entries.dsr_vidhan_ipc = ?) AND dsr_entries.sp_id = ? AND pidit_list.pidit_caste = ? AND (dsr_entries.dsr_kaymi_date >= ? and dsr_entries.dsr_kaymi_date <= ?) AND (dsr_status.s_status = ? OR dsr_status.s_status = ?) AND dsr_entries.atyachar_adhiniyam = ? ") )
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt10->bind_param("iiissssss", $j, $k, $branch_id, $sc, $datef, $date1, $kharji, $kharji1, $atyachar) )
    echo "Binding Parameter Error: ($police_dsr->errno) $police_dsr->error";	
	}
	else
	{
...
$ipc8[$i]=$cid9;	
	...
	if ($i===13)
	{
	if ( !$stmt11 = $police_dsr->prepare("SELECT COUNT(DISTINCT(pidit_list.dsr_id)) FROM dsr_status INNER JOIN pidit_list ON dsr_status.dsr_id = pidit_list.dsr_id INNER JOIN dsr_entries ON pidit_list.dsr_id = dsr_entries.id WHERE (dsr_entries.dsr_vidhan_ipc = ? OR dsr_entries.dsr_vidhan_ipc = ?) AND dsr_entries.sp_id = ? AND pidit_list.pidit_caste = ? AND (dsr_entries.dsr_kaymi_date >= ? and dsr_entries.dsr_kaymi_date <= ?) AND (dsr_status.s_status = ? OR dsr_status.s_status = ?) AND dsr_entries.atyachar_adhiniyam = ? ") )
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt11->bind_param("iiissssss", $j, $k, $branch_id, $st, $datef, $date1, $kharji, $kharji1, $atyachar) )
    echo "Binding Parameter Error: ($police_dsr->errno) $police_dsr->error";	
	}
	else
	{
...
$ipc9[$i]=$cid10;	
	...
	if ($i===13)
	{
	if ( !$stmt12 = $police_dsr->prepare("SELECT COUNT(DISTINCT(pidit_list.dsr_id)) FROM dsr_status INNER JOIN pidit_list ON dsr_status.dsr_id = pidit_list.dsr_id INNER JOIN dsr_entries ON pidit_list.dsr_id = dsr_entries.id WHERE (dsr_entries.dsr_vidhan_ipc = ? OR dsr_entries.dsr_vidhan_ipc = ?) AND dsr_entries.sp_id = ? AND pidit_list.pidit_caste = ? AND (dsr_entries.dsr_kaymi_date >= ? and dsr_entries.dsr_kaymi_date <= ?) AND dsr_status.s_status = ? AND dsr_entries.atyachar_adhiniyam = ? ") )
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt12->bind_param("iiisssss", $j, $k, $branch_id, $st, $datef, $date1, $nyay, $atyachar) )
    echo "Binding Parameter Error: ($police_dsr->errno) $police_dsr->error";	
	}
	else
	{
...
$ipc10[$i]=$cid11;	
	...
	if ($i===13)
	{
	if ( !$stmt13 = $police_dsr->prepare("SELECT COUNT(DISTINCT(pidit_list.dsr_id)) FROM dsr_status INNER JOIN pidit_list ON dsr_status.dsr_id = pidit_list.dsr_id INNER JOIN dsr_entries ON pidit_list.dsr_id = dsr_entries.id WHERE (dsr_entries.dsr_vidhan_ipc = ? OR dsr_entries.dsr_vidhan_ipc = ?) AND dsr_entries.sp_id = ? AND pidit_list.pidit_caste = ? AND (dsr_entries.dsr_kaymi_date >= ? and dsr_entries.dsr_kaymi_date <= ?) AND dsr_status.s_status = ? AND dsr_entries.atyachar_adhiniyam = ? ") )
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt13->bind_param("iiisssss", $j, $k, $branch_id, $st, $datef, $date1, $nyay, $atyachar) )
    echo "Binding Parameter Error: ($police_dsr->errno) $police_dsr->error";	
	}
	else
	{
...
$ipc11[$i]=$cid12;	
	...
	if ($i===13)
	{
	if ( !$stmt14 = $police_dsr->prepare("SELECT COUNT(DISTINCT(pidit_list.dsr_id)) FROM dsr_status INNER JOIN pidit_list ON dsr_status.dsr_id = pidit_list.dsr_id INNER JOIN dsr_entries ON pidit_list.dsr_id = dsr_entries.id WHERE (dsr_entries.dsr_vidhan_ipc = ? OR dsr_entries.dsr_vidhan_ipc = ?) AND dsr_entries.sp_id = ? AND pidit_list.pidit_caste = ? AND (dsr_entries.dsr_kaymi_date >= ? and dsr_entries.dsr_kaymi_date <= ?) AND dsr_status.s_decision = ? AND dsr_entries.atyachar_adhiniyam = ? ") )
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt14->bind_param("iiisssss", $j, $k, $branch_id, $st, $datef, $date1, $dandit, $atyachar) )
    echo "Binding Parameter Error: ($police_dsr->errno) $police_dsr->error";	
	}
	else
	{
...
$ipc12[$i]=$cid13;	
	...
	if ($i===13)
	{
	if ( !$stmt15 = $police_dsr->prepare("SELECT COUNT(DISTINCT(pidit_list.dsr_id)) FROM dsr_status INNER JOIN pidit_list ON dsr_status.dsr_id = pidit_list.dsr_id INNER JOIN dsr_entries ON pidit_list.dsr_id = dsr_entries.id WHERE (dsr_entries.dsr_vidhan_ipc = ? OR dsr_entries.dsr_vidhan_ipc = ?) AND dsr_entries.sp_id = ? AND pidit_list.pidit_caste = ? AND (dsr_entries.dsr_kaymi_date >= ? and dsr_entries.dsr_kaymi_date <= ?) AND dsr_status.s_decision = ? AND dsr_entries.atyachar_adhiniyam = ? ") )
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt15->bind_param("iiisssss", $j, $k, $branch_id, $st, $datef, $date1, $dandit, $atyachar) )
    echo "Binding Parameter Error: ($police_dsr->errno) $police_dsr->error";	
	}
	else
	{
...
$ipc13[$i]=$cid14;	
    ...
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt16->bind_param("iiisssss", $j, $k, $branch_id, $st, $datef, $date1, $doshmukt, $atyachar) )
    echo "Binding Parameter Error: ($police_dsr->errno) $police_dsr->error";	
	}
	else
	{
...
$ipc14[$i]=$cid15;	
	...
	if ($i===13)
	{
	if ( !$stmt17 = $police_dsr->prepare("SELECT COUNT(DISTINCT(pidit_list.dsr_id)) FROM dsr_status INNER JOIN pidit_list ON dsr_status.dsr_id = pidit_list.dsr_id INNER JOIN dsr_entries ON pidit_list.dsr_id = dsr_entries.id WHERE (dsr_entries.dsr_vidhan_ipc = ? OR dsr_entries.dsr_vidhan_ipc = ?) AND dsr_entries.sp_id = ? AND pidit_list.pidit_caste = ? AND (dsr_entries.dsr_kaymi_date >= ? and dsr_entries.dsr_kaymi_date <= ?) AND dsr_status.s_decision = ? AND dsr_entries.atyachar_adhiniyam = ? ") )
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt17->bind_param("iiisssss", $j, $k, $branch_id, $st, $datef, $date1, $doshmukt, $atyachar) )
    echo "Binding Parameter Error: ($police_dsr->errno) $police_dsr->error";	
	}
	else
	{
...
$ipc15[$i]=$cid16;	
	...
	?>
		...
		if ($i===13)
	    {
	    if ( !$stmt18 = $police_dsr->prepare("SELECT COUNT(DISTINCT(pidit_list.dsr_id)) FROM dsr_status INNER JOIN pidit_list ON dsr_status.dsr_id = pidit_list.dsr_id INNER JOIN dsr_entries ON pidit_list.dsr_id = dsr_entries.id WHERE (dsr_entries.dsr_vidhan_ipc = ? OR dsr_entries.dsr_vidhan_ipc = ?) AND dsr_entries.sp_id = ? AND pidit_list.pidit_caste = ? AND (dsr_entries.dsr_kaymi_date >= ? and dsr_entries.dsr_kaymi_date <= ?) AND dsr_status.s_status = ? AND dsr_entries.atyachar_adhiniyam = ? AND dsr_status.status != ?") )
        echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
        if ( !$stmt18->bind_param("iiissssss", $j, $k, $branch_id, $st, $datef, $date1, $nyay_lambit, $atyachar, $inactive) )
        echo "Binding Parameter Error: ($police_dsr->errno) $police_dsr->error";	
	    }
	   else
	    {
        ...
        echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
        if ( !$stmt19->bind_param("iiissssss", $j, $k, $branch_id, $st, $datef, $date1, $nyay_lambit, $atyachar, $inactive) )
        echo "Binding Parameter Error: ($police_dsr->errno) $police_dsr->error";	
	    }
	   else
	    {

...
<table border="1" cellspacing="0" cellpadding="5" style="font-size:12px;line-height:20px;margin-left:-30px;text-align:center" id="print" width="960px">
  ...
  <tr>
	...
	$dsr_balig1 ='वयस्क';
	$dsr_gender ='महिला';
	if ( !$stmt2 = $police_dsr->prepare("SELECT COUNT(id) FROM dsr_entries WHERE sp_id = ? AND (dsr_kaymi_date >= ? and dsr_kaymi_date <= ?) AND (dsr_vidhan_ipc = ? OR dsr_vidhan_ipc = ?) AND dsr_gender = ? AND dsr_balig = ? ") ) 
    ...
    echo "Binding Parameter Error: ($police_dsr->errno) $police_dsr->error";
		...
		$dsr_kdnp_rsn ='फिरौती के लिये';
		$dsr_gender ='महिला';
	if ( !$stmt2 = $police_dsr->prepare("SELECT COUNT(id) FROM dsr_entries WHERE sp_id = ? AND (dsr_kaymi_date >= ? and dsr_kaymi_date <= ?) AND (dsr_vidhan_ipc = ? OR dsr_vidhan_ipc = ?) AND dsr_kdnp_rsn != ? AND dsr_gender != ? ") ) 
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt2->bind_param("issssss", $branch_id1, $datef, $date1, $j, $k, $dsr_kdnp_rsn, $dsr_gender) )
    echo "Binding Parameter Error: ($police_dsr->errno) $police_dsr->error";	
		}
	else
	{
	...
	$dsr_balig1 ='वयस्क';
	$dsr_gender ='महिला';
    if ( !$stmt17 = $police_dsr->prepare("SELECT COUNT(id) FROM dsr_entries WHERE sp_id = ? AND (dsr_kaymi_date >= ? and dsr_kaymi_date <= ?) AND (dsr_vidhan_ipc = ? OR dsr_vidhan_ipc = ?) AND dsr_gender = ? AND dsr_balig = ?") ) 
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
    ...
    echo "Binding Parameter Error: ($police_dsr->errno) $police_dsr->error";
		...
		$dsr_kdnp_rsn ='फिरौती के लिये';
		$dsr_gender ='महिला';
	 if ( !$stmt17 = $police_dsr->prepare("SELECT COUNT(id) FROM dsr_entries WHERE sp_id = ? AND (dsr_kaymi_date >= ? and dsr_kaymi_date <= ?) AND (dsr_vidhan_ipc = ? OR dsr_vidhan_ipc = ?) AND dsr_kdnp_rsn != ? AND dsr_gender != ? ") ) 
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt17->bind_param("issssss", $branch_id1, $newDate1, $newDate2, $j, $k, $dsr_kdnp_rsn, $dsr_gender) )
    echo "Binding Parameter Error: ($police_dsr->errno) $police_dsr->error";
		}
		else{
	...
	$dsr_balig1 ='वयस्क';
	$dsr_gender ='महिला';
    if ( !$stmt18 = $police_dsr->prepare("SELECT COUNT(id) FROM dsr_entries WHERE sp_id = ? AND (dsr_kaymi_date >= ? and dsr_kaymi_date <= ?) AND (dsr_vidhan_ipc = ? OR dsr_vidhan_ipc = ?) AND dsr_gender = ? AND dsr_balig = ?") ) 
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
    ...
    echo "Binding Parameter Error: ($police_dsr->errno) $police_dsr->error";
		...
		$dsr_kdnp_rsn ='फिरौती के लिये';
		$dsr_gender ='महिला';
	 if ( !$stmt18 = $police_dsr->prepare("SELECT COUNT(id) FROM dsr_entries WHERE sp_id = ? AND (dsr_kaymi_date >= ? and dsr_kaymi_date <= ?) AND (dsr_vidhan_ipc = ? OR dsr_vidhan_ipc = ?) AND dsr_kdnp_rsn != ? AND dsr_gender != ? ") ) 
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt18->bind_param("issssss", $branch_id1, $newDate3, $newDate4, $j, $k, $dsr_kdnp_rsn, $dsr_gender) )
    echo "Binding Parameter Error: ($police_dsr->errno) $police_dsr->error";
		}
		else{

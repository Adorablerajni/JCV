...
<p align="right"></p>
  ...
  ?>
	...
	if ( !$stmt2 = $police_dsr->prepare("SELECT COUNT(id) FROM dsr_entries WHERE sp_id = ? AND dsr_kaymi_date < ? AND dsr_vidhan_ipc=?") ) 
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt2->bind_param("iss",$branch_id,$datef,$dsr_vidhan_ipc) )
	...
	if ( !$stmt3 = $police_dsr->prepare("SELECT COUNT(dsr_entries.id) FROM criminal_list INNER JOIN dsr_entries ON criminal_list.dsr_id= dsr_entries.id WHERE dsr_entries.sp_id = ? AND dsr_entries.dsr_kaymi_date<? AND dsr_entries.dsr_vidhan_ipc=?") ) 
		
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt3->bind_param("iss",$branch_id,$datef,$dsr_vidhan_ipc) )
    echo "Binding Parameter Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt3->execute() ) 
    ...
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt4->bind_param("isss",$branch_id,$datef, $date1,$dsr_vidhan_ipc) )
    echo "Binding Parameter Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt4->execute() ) 
	...
	<span><?php echo $cid4;?></span></td>
        <td><?php
	  $police_dsr->select_db("ftcaaazc_dsr");
	if ( !$stmt5 = $police_dsr->prepare("SELECT COUNT(dsr_entries.id) FROM criminal_list INNER JOIN dsr_entries ON criminal_list.dsr_id= dsr_entries.id WHERE dsr_entries.sp_id = ? AND dsr_entries.(dsr_kaymi_date >= ? and dsr_kaymi_date <= ?) AND dsr_entries.dsr_vidhan_ipc=?") ) 
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt5->bind_param("isss",$branch_id,$datef, $date1,$dsr_vidhan_ipc) )
    echo "Binding Parameter Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt5->execute() ) 
	...
	<span><?php echo $cid5;?></span></td>
        <td><?php
	  $police_dsr->select_db("ftcaaazc_dsr");
	if ( !$stmt6 = $police_dsr->prepare("SELECT COUNT(id) FROM dsr_entries WHERE sp_id = ? AND (dsr_kaymi_date >= ? and dsr_kaymi_date <= ?) AND dsr_vidhan_ipc=? AND c_status=?") ) 
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt6->bind_param("issss",$branch_id,$datef, $date1,$dsr_vidhan_ipc,$c_status) )
    echo "Binding Parameter Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt6->execute() ) 
	...
	<span><?php echo $cid6;?></span></td>
        <td><?php
	  $police_dsr->select_db("ftcaaazc_dsr");
	if ( !$stmt7 = $police_dsr->prepare("SELECT COUNT(dsr_entries.id) FROM criminal_list INNER JOIN dsr_entries ON criminal_list.dsr_id= dsr_entries.id WHERE dsr_entries.sp_id = ? AND dsr_entries.(dsr_kaymi_date >= ? and dsr_kaymi_date <= ?) AND dsr_entries.dsr_vidhan_ipc=? AND dsr_entries.c_status=?") ) 
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt7->bind_param("issss",$branch_id,$datef, $date1,$dsr_vidhan_ipc,$c_status) )
    echo "Binding Parameter Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt7->execute() ) 
	...
	<span><?php echo $cid7;?></span></td>
        <td><?php
	  $police_dsr->select_db("ftcaaazc_dsr");
	if ( !$stmt8 = $police_dsr->prepare("SELECT COUNT(id) FROM dsr_entries WHERE sp_id = ? AND dsr_kaymi_date>? AND dsr_vidhan_ipc=?") ) 
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt8->bind_param("iss",$branch_id,$date1,$dsr_vidhan_ipc) )
    echo "Binding Parameter Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt8->execute() ) 
	...
	<span><?php echo $cid8;?></span></td>
        <td><?php
	  $police_dsr->select_db("ftcaaazc_dsr");
	if ( !$stmt9 = $police_dsr->prepare("SELECT COUNT(dsr_entries.id) FROM criminal_list INNER JOIN dsr_entries ON criminal_list.dsr_id= dsr_entries.id WHERE dsr_entries.sp_id = ? AND dsr_entries.dsr_kaymi_date>? AND dsr_entries.dsr_vidhan_ipc=?") ) 
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt9->bind_param("iss",$branch_id,$date1,$dsr_vidhan_ipc) )
    echo "Binding Parameter Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt9->execute() ) 
    ...
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt10->bind_param("isss",$branch_id,$date, $date2,$dsr_vidhan_ipc) )
    echo "Binding Parameter Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt10->execute() ) 
	...
	<span><?php echo $cid10;?></span></td>
        <td><?php
	  $police_dsr->select_db("ftcaaazc_dsr");
	if ( !$stmt11 = $police_dsr->prepare("SELECT COUNT(dsr_entries.id) FROM criminal_list INNER JOIN dsr_entries ON criminal_list.dsr_id= dsr_entries.id WHERE dsr_entries.sp_id = ? AND dsr_entries.(dsr_kaymi_date >= ? and dsr_kaymi_date <= ?) AND dsr_entries.dsr_vidhan_ipc=?") ) 
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt11->bind_param("isss",$branch_id,$date, $date2,$dsr_vidhan_ipc) )
    echo "Binding Parameter Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt11->execute() ) 
	...
	<span><?php echo $cid11;?></span></td>
      ...
      <tr>
        ...
        <td class="" colspan="2">जोन का योग</td>
          <td><?php
	  $police_dsr->select_db("ftcaaazc_dsr");
	if ( !$stmt12 = $police_dsr->prepare("SELECT COUNT(id) FROM dsr_entries WHERE dsr_kaymi_date < ? AND dsr_vidhan_ipc=?") ) 
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt12->bind_param("ss",$datef,$dsr_vidhan_ipc) )
    echo "Binding Parameter Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt12->execute() ) 
	...
	<span><?php echo $cid12;?></span></td>
        <td class=""><?php
	  $police_dsr->select_db("ftcaaazc_dsr");
	if ( !$stmt13 = $police_dsr->prepare("SELECT COUNT(dsr_entries.id) FROM criminal_list INNER JOIN dsr_entries ON criminal_list.dsr_id= dsr_entries.id WHERE dsr_entries.dsr_kaymi_date<? AND dsr_entries.dsr_vidhan_ipc=?") ) 
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt13->bind_param("ss",$datef,$dsr_vidhan_ipc) )
    echo "Binding Parameter Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt13->execute() ) 
	...
	<span><?php echo $cid13;?></span></td>
		...
		<?php
	  $police_dsr->select_db("ftcaaazc_dsr");
	if ( !$stmt14 = $police_dsr->prepare("SELECT COUNT(id) FROM dsr_entries WHERE  (dsr_kaymi_date >= ? and dsr_kaymi_date <= ?) AND dsr_vidhan_ipc=?") ) 
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt14->bind_param("sss",$datef, $date1,$dsr_vidhan_ipc) )
    echo "Binding Parameter Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt14->execute() ) 
	...
	<span><?php echo $cid14;?></span></td>
        <td><?php
	  $police_dsr->select_db("ftcaaazc_dsr");
	if ( !$stmt15 = $police_dsr->prepare("SELECT COUNT(dsr_entries.id) FROM criminal_list INNER JOIN dsr_entries ON criminal_list.dsr_id= dsr_entries.id WHERE dsr_entries.(dsr_kaymi_date >= ? and dsr_kaymi_date <= ?) AND dsr_entries.dsr_vidhan_ipc=?") ) 
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt15->bind_param("sss",$datef, $date1,$dsr_vidhan_ipc) )
    echo "Binding Parameter Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt15->execute() ) 
	...
	<span><?php echo $cid15;?></span></td>
        <td><?php
	  $police_dsr->select_db("ftcaaazc_dsr");
	if ( !$stmt16 = $police_dsr->prepare("SELECT COUNT(id) FROM dsr_entries WHERE (dsr_kaymi_date >= ? and dsr_kaymi_date <= ?) AND dsr_vidhan_ipc=? AND c_status=?") ) 
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt16->bind_param("ssss",$datef, $date1,$dsr_vidhan_ipc,$c_status) )
    echo "Binding Parameter Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt16->execute() ) 
	...
	<span><?php echo $cid16;?></span></td>
        <td><?php
	  $police_dsr->select_db("ftcaaazc_dsr");
	if ( !$stmt17 = $police_dsr->prepare("SELECT COUNT(dsr_entries.id) FROM criminal_list INNER JOIN dsr_entries ON criminal_list.dsr_id= dsr_entries.id WHERE dsr_entries.(dsr_kaymi_date >= ? and dsr_kaymi_date <= ?) AND dsr_entries.dsr_vidhan_ipc=? AND dsr_entries.c_status=?") ) 
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt17->bind_param("ssss",$datef, $date1,$dsr_vidhan_ipc,$c_status) )
    echo "Binding Parameter Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt17->execute() ) 
	...
	<span><?php echo $cid17;?></span></td>
        <td><?php
	  $police_dsr->select_db("ftcaaazc_dsr");
	if ( !$stmt18 = $police_dsr->prepare("SELECT COUNT(id) FROM dsr_entries WHERE dsr_kaymi_date>? AND dsr_vidhan_ipc=?") ) 
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt18->bind_param("ss", $date1,$dsr_vidhan_ipc) )
    echo "Binding Parameter Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt18->execute() ) 
	...
	<span><?php echo $cid18;?></span></td>
        <td><?php
	  $police_dsr->select_db("ftcaaazc_dsr");
	if ( !$stmt19 = $police_dsr->prepare("SELECT COUNT(dsr_entries.id) FROM criminal_list INNER JOIN dsr_entries ON criminal_list.dsr_id= dsr_entries.id WHERE dsr_entries.dsr_kaymi_date>? AND dsr_entries.dsr_vidhan_ipc=?") ) 
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt19->bind_param("ss",$date1,$dsr_vidhan_ipc) )
    echo "Binding Parameter Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt19->execute() ) 
	...
	<span><?php echo $cid19;?></span></td>
        <td><?php
	  $police_dsr->select_db("ftcaaazc_dsr");
	if ( !$stmt20 = $police_dsr->prepare("SELECT COUNT(id) FROM dsr_entries WHERE (dsr_kaymi_date >= ? and dsr_kaymi_date <= ?) AND dsr_vidhan_ipc=?") ) 
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt20->bind_param("sss",$date, $date2,$dsr_vidhan_ipc) )
    echo "Binding Parameter Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt20->execute() ) 
	...
	<span><?php echo $cid20;?></span></td>
        <td><?php
	  $police_dsr->select_db("ftcaaazc_dsr");
	if ( !$stmt21 = $police_dsr->prepare("SELECT COUNT(dsr_entries.id) FROM criminal_list INNER JOIN dsr_entries ON criminal_list.dsr_id= dsr_entries.id WHERE dsr_entries.(dsr_kaymi_date >= ? and dsr_kaymi_date <= ?) AND dsr_entries.dsr_vidhan_ipc=?") ) 
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt21->bind_param("sss",$date, $date2,$dsr_vidhan_ipc) )
    echo "Binding Parameter Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt21->execute() ) 

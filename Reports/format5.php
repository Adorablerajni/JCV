...
<title>Format-5 | File Tracking & Crime Analysis Application </title>
<!-- GLOBAL STYLES -->
...
<span style="float:right;margin-right:-25px"> <input type="hidden" id="datatodisplay" name="datatodisplay" style="" /> <input type="submit" value="Export to Excel" class="btn btn-warning"/>
 ...
 <div class="row">
    <div class="col-lg-12 label-primary" style="color:#FFF">
      <form action="format5.php" method="post" name="Search" enctype="multipart/form-data" id="popup-validation" role="form">
  ...
  <?php
   ...
   {
  ?>
<p align="left" style="float:left;margin-left:-30px;">Format-5</p>
<p align="right"></p>
  <div class="mar10">
  <?php   
  ...
  ?>
    <p align="center"><span>न्यायालयीन प्रकरणों की जानकारी दि. <?php if ($_POST['datef'] == ''){	echo '';} else{echo date('d-m-Y', strtotime($_POST['datef']));
 ...
 ?>
    <table border="1" cellspacing="0" cellpadding="5" style="text-align:center" id="print" width="960px" class="table table-striped table-bordered table-hover">
      ...
      <tr>
        ...
        <td class="" align="left"><?php echo $branch_name; echo ","; echo $city;?></td>
        
		<td>
		<?php
	  $police_dsr->select_db("ftcaaazc_dsr");
	  $c_status ='निराकृत';
	if ( !$stmt2 = $police_dsr->prepare("SELECT COUNT(DISTINCT(dsr_status.dsr_id)) as ds_id3 FROM dsr_status LEFT JOIN dsr_entries ON dsr_status.dsr_id = dsr_entries.id WHERE dsr_status.sp_id = ? AND (s_status_date between ? and ?) AND s_status = ?") ) 
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt2->bind_param("isss", $branch_id, $datef, $date1, $c_status) )
	...
	?>
	  ...
	  $police_dsr->select_db("ftcaaazc_dsr");
	  $c_decision='सजा';
	if ( !$stmt3 = $police_dsr->prepare("SELECT COUNT(DISTINCT(dsr_status.dsr_id)) as ds_id3 FROM dsr_status LEFT JOIN dsr_entries ON dsr_status.dsr_id = dsr_entries.id WHERE dsr_status.sp_id = ? AND (s_status_date between ? and ?) AND s_decision = ?") ) 
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt3->bind_param("isss",$branch_id,$datef, $date1,$c_decision) )
    echo "Binding Parameter Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt3->execute() ) 
...
echo $percent_friendly7 ;
      ...
      <tr>
	    <td></td>
        <td class="">जोन का योग</td>
		
        <td>	
		<?php
	  $police_dsr->select_db("ftcaaazc_dsr");
	  $c_status ='निराकृत';
	if ( !$stmt4 = $police_dsr->prepare("SELECT COUNT(DISTINCT(dsr_status.dsr_id)) as ds_id3 FROM dsr_status LEFT JOIN dsr_entries ON dsr_status.dsr_id = dsr_entries.id WHERE (s_status_date between ? and ?) AND s_status = ?") ) 
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt4->bind_param("sss", $datef, $date1, $c_status) )
    echo "Binding Parameter Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt4->execute() ) 
	...
	?>
	  ...
	  $police_dsr->select_db("ftcaaazc_dsr");
	  $c_decision='सजा';
	if ( !$stmt5 = $police_dsr->prepare("SELECT COUNT(DISTINCT(dsr_status.dsr_id)) as ds_id3 FROM dsr_status LEFT JOIN dsr_entries ON dsr_status.dsr_id = dsr_entries.id WHERE (s_status_date between ? and ?) AND s_decision = ?") ) 
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt5->bind_param("sss",$datef, $date1,$c_decision) )
    echo "Binding Parameter Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt5->execute() ) 

...
$ipc6[$j]=$cid7;	
	...
	$four_wheeler = '70';
	//$theft1 = '9';
	if ( !$stmt9 = $police_dsr->prepare("SELECT COUNT(id) FROM dsr_entries WHERE sp_id = ? AND (dsr_kaymi_date >= ? and dsr_kaymi_date <= ?) AND (dsr_vidhan_ipc = ? or dsr_vidhan_ipc = ? or dsr_vidhan_ipc = ?)") ) 
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt9->bind_param("issiii", $idcheck, $datef, $date1, $theft, $two_wheeler, $four_wheeler) )
    echo "Binding Parameter Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt9->execute() ) 
...
$ipc10[$j]=$cid11;	
	...
	$molestation = '15';
	$k='39';
	if ( !$stmt13= $police_dsr->prepare("SELECT COUNT(id) FROM dsr_entries WHERE sp_id = ? AND (dsr_kaymi_date >= ? and dsr_kaymi_date <= ?) AND (dsr_vidhan_ipc = ? or dsr_vidhan_ipc = ? ) ") ) 
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt13->bind_param("issii", $idcheck, $datef, $date1, $molestation, $k) )
    echo "Binding Parameter Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt13->execute() ) 
...
$ipc11[$j]=$cid12;	
	...
	$police_dsr->select_db("ftcaaazc_dsr");	
	$gum = '102';
	if ( !$stmt14 = $police_dsr->prepare("SELECT COUNT(id) FROM dsr_entries where sp_id = ? and dsr_vidhan_ipc is not NULL and dsr_vidhan_ipc != '' AND dsr_vidhan_ipc != ? AND (dsr_kaymi_date >= ? and dsr_kaymi_date <= ?)") ) //
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt14->bind_param("iiss",$idcheck,$gum, $datef, $date1))//
    echo "Binding Parameter Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt14->execute() ) 
    ...
    $stmt14->bind_result($cid23);
	$stmt14->fetch();
	
$cid13=$cid23-($cid1+$cid2+$cid3+$cid4+$cid5+$cid6+$cid7+$cid8+$cid9+$cid10+$cid11+$cid12);
$ipc12[$j]=$cid13;
    ?>
	...
	if ( !$stmt15= $police_dsr->prepare("SELECT COUNT(id) FROM dsr_entries WHERE sp_id = ? AND (dsr_kaymi_date >= ? and dsr_kaymi_date <= ?) AND (dsr_vidhan_ipc = ? OR dsr_vidhan_ipc = ?) AND dsr_kdnp_rsn = ?") ) 
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt15->bind_param("issiis", $idcheck, $datef, $date1, $kidnapping_child, $kdnp, $kidnapping_ranson) )
    echo "Binding Parameter Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt15->execute() ) 
	...
	if ( !$stmt16= $police_dsr->prepare("SELECT COUNT(id) FROM dsr_entries WHERE sp_id = ? AND (dsr_kaymi_date >= ? and dsr_kaymi_date <= ?) AND (dsr_vidhan_ipc = ? OR dsr_vidhan_ipc = ?) AND dsr_kdnp_rsn = ?") ) 
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt16->bind_param("issiis", $idcheck, $datef, $date1, $kidnapping_child, $kdnp, $kidnapping_begging) )
    echo "Binding Parameter Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt16->execute() ) 
...
$ipc15[$j]=$cid15;	
	...
	$kdnp ='13';
	if ( !$stmt17= $police_dsr->prepare("SELECT COUNT(id) FROM dsr_entries WHERE sp_id = ? AND (dsr_kaymi_date >= ? and dsr_kaymi_date <= ?) AND (dsr_vidhan_ipc = ? OR dsr_vidhan_ipc = ?) AND dsr_gender = ?") ) 
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt17->bind_param("issiis", $idcheck, $datef, $date1, $kidnapping_child, $kdnp, $kidnapping_girl) )
    echo "Binding Parameter Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt17->execute() ) 
...
$ipc16[$j]=$cid16;	
	...
	$kidnapping_ranson = 'फिरौती के लिये';
	$kdnp ='13';
	if ( !$stmt18= $police_dsr->prepare("SELECT COUNT(id) FROM dsr_entries WHERE sp_id = ? AND ((dsr_kaymi_date >= ? and dsr_kaymi_date <= ?)) AND (dsr_vidhan_ipc = ? OR dsr_vidhan_ipc = ?) AND (dsr_kdnp_rsn != ? OR dsr_kdnp_rsn != ?) and dsr_gender !=?") ) 
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt18->bind_param("issiisss", $idcheck, $datef, $date1, $kidnapping_child, $kdnp, $kidnapping_begging, $kidnapping_ranson, $kidnapping_girl) )
    echo "Binding Parameter Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt18->execute() ) 
...
$ipc17[$j]=$cid17;	
	...
	<?php
 	$police_dsr->select_db("ftcaaazc_dsr");
        ...
        $a7='6';
		$a8='8';
	if ( !$stmt20= $police_dsr->prepare("SELECT SUM(dsr_theft_amount) FROM dsr_entries WHERE sp_id = ? AND (dsr_kaymi_date >= ? and dsr_kaymi_date <= ?) AND (dsr_vidhan_ipc = ? OR dsr_vidhan_ipc = ? OR dsr_vidhan_ipc = ? OR dsr_vidhan_ipc = ? OR dsr_vidhan_ipc = ? OR dsr_vidhan_ipc = ? OR dsr_vidhan_ipc = ? OR dsr_vidhan_ipc = ?)") ) 
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt20->bind_param("issiiiiiiii", $idcheck, $datef, $date1, $a1, $a2, $a3, $a4, $a5, $a6, $a7, $a8) )
    echo "Binding Parameter Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt20->execute() ) 
    echo "Execute Error: ($stmt20->errno)  $stmt20->error";
    if ( !$stmt20->store_result() ) 
    echo "Storing Result Error: ($police_dsr->errno) $police_dsr->error";
	...
	//echo $dsr_id1;
 	$police_dsr->select_db("ftcaaazc_dsr");
        ...
        $a7='6';
		$a8='8';
	if ( !$stmt21= $police_dsr->prepare("SELECT SUM(dsr_seized_amnt) FROM dsr_entries WHERE sp_id = ? AND (dsr_kaymi_date >= ? and dsr_kaymi_date <= ?) AND (dsr_vidhan_ipc = ? OR dsr_vidhan_ipc = ? OR dsr_vidhan_ipc = ? OR dsr_vidhan_ipc = ? OR dsr_vidhan_ipc = ? OR dsr_vidhan_ipc = ? OR dsr_vidhan_ipc = ? OR dsr_vidhan_ipc = ?)") ) //AND property_date BETWEEN ? AND ? AND dsr_seized_amnt != ? 
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt21->bind_param("issiiiiiiii", $idcheck, $datef, $date1, $a1, $a2, $a3, $a4, $a5, $a6, $a7, $a8) ) //, $datef, $date1
    echo "Binding Parameter Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt21->execute() ) 
...
$ipc19[$j]=$cid19;	
    ?>
	<span><?php if ($cid19 > 0){echo $cid19;}else{echo 0;}?></span>
	<?php 
	$stmt21->close();
	?>
	</td>

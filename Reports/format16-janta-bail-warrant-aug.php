...
<title>Format16-janta-bail-warrant | File Tracking & Crime Analysis Application </title>
<!-- GLOBAL STYLES -->
...
<span style="float:right;margin-right:-25px"> <input type="hidden" id="datatodisplay" name="datatodisplay" style="" /> <input type="submit" value="Export to Excel"/>
  ...
  <div class="row">
    <div class="col-lg-12">
      <form action="format16-janta-bail-warrant.php" method="post" name="Search" enctype="multipart/form-data" id="popup-validation" role="form">
...
<p align="right"></p>
  ...
  ?>
    <p align="center"><span>जनता जमानती वारंटः- दि. <?php if ($_POST['datef'] == ''){	echo '';} else{echo date('d-m-Y', strtotime($_POST['datef']));
 ...
 ?>
   <table border="1" cellspacing="0" cellpadding="5" style="font-size:12px;line-height:20px;margin-left:-30px;text-align:center" id="print" width="960px">
	...
	$tamil_date='';
	$sub_id='No';
	if ( !$stmt2 = $police_summons->prepare("SELECT COUNT(id) FROM summon_list WHERE sp_id = ? AND warrant_type = ? AND ((samans_date < ? AND tamil_date is null ) OR  (tamil_date is not null AND tamil_date >= ? AND samans_date < ?)) AND sub_id = ? AND aaropi_type = ?") )  //SELECT COUNT(id) FROM summon_list WHERE sp_id = ? AND warrant_type = ? AND ((creation_date < ? OR tamil_date > ? ) OR (creation_date < ? AND (tamil_date is null OR tamil_date = ? ))) AND sub_id=? AND aaropi_type=?
	...
	$aaropi_type='जनता';
	$sub_id='No';
	if ( !$stmt4 = $police_summons->prepare("SELECT COUNT(id) FROM summon_list WHERE sp_id = ? AND warrant_type = ? AND tamil_date BETWEEN ? AND ? AND sub_id=? AND aaropi_type=?") ) 
    echo "Prepare Error: ($police_summons->errno) $police_summons->error";

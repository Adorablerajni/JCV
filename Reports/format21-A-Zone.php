...
<title>Format-21-A Zone | File Tracking & Crime Analysis Application </title>
<!-- GLOBAL STYLES -->
...
<span style="float:right;margin-right:-25px"> <input type="hidden" id="datatodisplay" name="datatodisplay" style="" /> <input type="submit" value="Export to Excel" class="btn btn-warning"/>
 ...
 <div class="row">
    <div class="col-lg-12 label-primary" style="color:#FFF">
      <form action="format21-A-Zone.php" method="post" name="Search" enctype="multipart/form-data" id="popup-validation" role="form">
...
<div class="notice_all">
  ...
  {
  ?>
  <p align="left" style="float:left;margin-left:-30px;">Format-21-A</p>
  <p align="right"></p>
  <div class="mar10">
  <?php
  $police_tracking->select_db("ftcaaazc_epfts");
...
<table border="1" cellspacing="0" cellpadding="5" style="text-align:center" id="print" width="960px" class="table table-striped table-bordered table-hover">
  ...
  <tr>
	...
	if ($i===4)
	{
	if ( !$stmt2 = $police_dsr->prepare("SELECT COUNT(id) FROM dsr_entries WHERE (dsr_vidhan_ipc = ? OR dsr_vidhan_ipc = ?) AND dsr_gender = ? AND  (dsr_kaymi_date >= ? and dsr_kaymi_date <= ?) AND dsr_caste = ?") ) 
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt2->bind_param("iissss", $j, $k, $gender, $datef, $date1, $st) )
    echo "Binding Parameter Error: ($police_dsr->errno) $police_dsr->error";
	}
	else
	{
	if ( !$stmt2 = $police_dsr->prepare("SELECT COUNT(id) FROM dsr_entries WHERE dsr_vidhan_ipc = ? AND dsr_gender = ? AND  (dsr_kaymi_date >= ? and dsr_kaymi_date <= ?) AND dsr_caste = ?") ) 
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt2->bind_param("issss", $j, $gender, $datef, $date1, $st) )
    echo "Binding Parameter Error: ($police_dsr->errno) $police_dsr->error";	
	}
    if ( !$stmt2->execute() ) 
  ...
  <tr>
	...
	<?php
 	$police_dsr->select_db("ftcaaazc_dsr");
	if ( !$stmt12 = $police_dsr->prepare("SELECT COUNT(id) FROM dsr_entries WHERE dsr_vidhan_minor = ? AND dsr_gender = ? AND  (dsr_kaymi_date >= ? and dsr_kaymi_date <= ?) AND dsr_caste = ?") ) 
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt12->bind_param("issss", $n, $gender, $datef, $date1, $st) )
    echo "Binding Parameter Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt12->execute() ) 

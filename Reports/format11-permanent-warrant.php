...
<title>Format11-permanent-warrant | File Tracking & Crime Analysis Application </title>
<!-- GLOBAL STYLES -->
...
<span style="float:right;margin-right:-25px"> <input type="hidden" id="datatodisplay" name="datatodisplay" style="" /> <input type="submit" value="Export to Excel" class="btn btn-warning"/>
 ...
 <div class="row">
    <div class="col-lg-12 label-primary" style="color:#FFF">
      <form action="format11-permanent-warrant.php" method="post" name="Search" enctype="multipart/form-data" id="popup-validation" role="form">
...
<div class="notice_all">
  ...
  <?php
   ...
   {
  ?>
<p align="left" style="float:left;margin-left:-30px;">Format11-permanent-warrant</p>
<p align="right"></p>
  <div class="mar10">
  <?php   
  ...
  ?>
    <p align="center"><span>स्थाई वारंट - दि. <?php if ($_POST['datef'] == ''){	echo '';} else{echo date('d-m-Y', strtotime($_POST['datef']));
...
$("#hideShow5<?php echo $aj; ?>").click(function(){
   $("#message5<?php echo $aj; ?>").toggle();
      ...
      <tr>
            ...
            $sub_id='No';
			$z=0;
	if ( !$stmt2 = $police_summons->prepare("SELECT id, sakshi_name FROM summon_list WHERE sp_id = ? AND warrant_type = ? AND samans_date < ? AND (((`tamil_date` <= `sakshi_peshi_date`) AND (sakshi_peshi_date > ?) and (tamil_date > ?)) OR ((`tamil_date` is null OR `tamil_date` ='') and `sakshi_peshi_date` > ?))AND (`transfer_date` is null OR `transfer_date` >= ?)  and `sub_id` = ?") )
    echo "Prepare Error: ($police_summons->errno) $police_summons->error";
    if ( !$stmt2->bind_param("isssssss",$branch_id, $warrant_type, $datef, $datef, $datef, $datef, $datef, $sub_id) )
	...
	<?php } ?>
            ...
            $sub_id='No';
           $a=0;
	if ( !$stmt3 = $police_summons->prepare("SELECT id, sakshi_name FROM summon_list WHERE sp_id = ? AND warrant_type = ? AND (samans_date between ? AND ?)  AND sub_id=?") ) //AND (tamil_date is null OR tamil_date = ?)
    echo "Prepare Error: ($police_summons->errno) $police_summons->error";
    ...
    $a++;	 
	?>             
	<a href="../Summon/get-search.php?sp=<?php echo $branch_id;?>&s_id=<?php echo $janta_smns_id1;?>"><?php echo $a;?>- <?php echo $sakshi_name1;?></a><br />
	<?php } ?>
	</div>
            </td>
    ...
    echo "Prepare Error: ($police_summons->errno) $police_summons->error";
    if ( !$stmt4->bind_param("issssssssss",$branch_id, $warrant_type, $datef, $datef, $datef, $datef, $datef, $date1, $sub_id, $datef, $date1) )
    echo "Binding Parameter Error: ($police_summons->errno) $police_summons->error";
    if ( !$stmt4->execute() ) 
    ...
    <?php } ?>
            ...
            $sub_id='No';
           $d=0;
	if ( !$stmt7 = $police_summons->prepare("SELECT id, sakshi_name FROM summon_list WHERE sp_id = ? AND warrant_type = ? AND ((`samans_date` < ? AND (((`tamil_date` <= `sakshi_peshi_date`) AND (sakshi_peshi_date > ?) and (tamil_date > ?)) OR ((`tamil_date` is null OR tamil_date ='') and `sakshi_peshi_date` > ?))) OR (`samans_date` between ? AND ?)) AND sub_id = ? AND (`transfer_date` between ? AND ?)") ) 
    echo "Prepare Error: ($police_summons->errno) $police_summons->error";
    if ( !$stmt7->bind_param("issssssssss",$branch_id, $warrant_type, $datef, $datef, $datef, $datef, $datef, $date1, $sub_id, $datef, $date1) )
    echo "Binding Parameter Error: ($police_summons->errno) $police_summons->error";
    if ( !$stmt7->execute() )  
    ...
    ?>
            ...
            echo "Prepare Error: ($police_summons->errno) $police_summons->error";
            if ( !$stmt6->bind_param("isss",$branch_id, $warrant_type,$sub_id,$tamil_date) )
            echo "Binding Parameter Error: ($police_summons->errno) $police_summons->error";
            if ( !$stmt6->execute() ) 
      ...
      <tr>
            ...
            <td>
            <?php $u=0;
            //$ipc4[1]+$ipc4[2]+$ipc4[3]+$ipc4[4]+$ipc4[5]+$ipc4[6]+$ipc4[7]+$ipc4[8]+$ipc4[9]+$ipc4[10];
            echo $u;?>
            </td>

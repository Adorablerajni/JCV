...
<title>Format-1-IPC(2) | File Tracking & Crime Analysis Application </title>
<!-- GLOBAL STYLES -->
...
<span style="float:right;margin-right:-25px"> <input type="hidden" id="datatodisplay" name="datatodisplay" style="" /> <input type="submit" value="Export to Excel" class="btn btn-warning"/>
 ...
 <div class="row">
    <div class="col-lg-12 label-primary" style="color:#FFF">
      <form action="format1-ipc2.php" method="post" name="Search" enctype="multipart/form-data" id="popup-validation" role="form">
...
<div class="">
  ...
  <?php
    ...
    {
  ?>
  <!--<p align="left" style="float:left;margin-left:-30px;">Format-1-ipc2</p>-->
  <p align="right"></p>
  <div class="mar10">
 <?php   
...
<table border="1" cellspacing="0" cellpadding="5" style="margin-left:30px;text-align:center" id="print" width="960px" class="table table-striped table-bordered table-hover">
  ...
  <tr>
	...
	<?php 
	  ...
	  $idcheck = $branch_id;
	?>
	<td colspan="4"><?php echo $branch_name; echo ","; echo $city; ?></td>    
    <?php } //stmt1 while end here?>
    <td colspan="4">जोन का योग</td>
  </tr>
  ...
  <tr>
    ...
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt2->bind_param("issi",$branch_id1,$datef, $date1,$j) )
    echo "Binding Parameter Error: ($police_dsr->errno) $police_dsr->error";
		}
    if ( !$stmt2->execute() ) 
    ...
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt17->bind_param("issi",$branch_id1,$newDate1,$newDate2,$j) )
    echo "Binding Parameter Error: ($police_dsr->errno) $police_dsr->error";
		}
    if ( !$stmt17->execute() ) 
    ...
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt18->bind_param("issi",$branch_id1,$newDate3, $newDate4,$j) )
    echo "Binding Parameter Error: ($police_dsr->errno) $police_dsr->error";
		}
    if ( !$stmt18->execute() ) 
...
?></td>
  ...
  <tr>
    ...
    $st2->bind_param("ss", $sp2, $_SESSION['MM_Zone']);
    $st2->execute();
    $st2->store_result();
	...
	$police_dsr->select_db("ftcaaazc_dsr");
	$gum = '102';
	if ( !$stmt18 = $police_dsr->prepare("SELECT SUM(crime) FROM old_records WHERE district = ? AND  ( start_date >= ? AND start_date <= ?) AND dsr_vidhan_ipc != ? AND dsr_vidhan_ipc != '' AND dsr_vidhan_ipc is not NULL") ) 
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt18->bind_param("isss", $branch_id2, $newDate3, $newDate4, $gum) )
    echo "Binding Parameter Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt18->execute() ) 
...
if($ip22[$l] > 0)
{	
$percentage22 =($ip12[$l]-$ip22[$l])*100/$ip22[$l] ;

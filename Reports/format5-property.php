...
<span style="float:right;margin-right:-25px"> <input type="hidden" id="datatodisplay" name="datatodisplay" style="" /> <input type="submit" value="Export to Excel" class="btn btn-warning"/>
 ...
 <div class="row">
    <div class="col-lg-12 label-primary" style="color:#FFF">
      <form action="format5-property.php" method="post" name="Search" enctype="multipart/form-data" id="popup-validation" role="form">
...
//   $newDate2 =date ("Y-m-d", strtotime ($datef ."-1 day"));
 ...
 ?>
 <br/>
    <table border="1" cellspacing="0" cellpadding="5" style="text-align:center" id="print" width="960px" class=" संपत्ति संबंधी अपराध में बरामदगी  table table-striped table-bordered table-hover">
      <tr>
        <td rowspan="2">क्र.</td>
	  ...
	  ?>
      <tr>
        <td class="j" ><?php echo $j; ?></td>
		
        <td class="<?php echo $branch_id; ?>"><?php echo $branch_name; echo ","; echo $city;?></td>
        ...
        */
        ...
        <td class="गत वर्ष का आलौच्य पक्ष  अपहृत">
            <?php
            $police_dsr->select_db("ftcaaazc_dsr");
            $a1='3';
            $a2='5';
            ...
            $a7='6';
            $a8='8';
            $last_year_datef =   date('Y-m-d', strtotime('-1 year', strtotime($datef)) );
            $last_year_date1 = date('Y-m-d', strtotime('-1 year', strtotime($date1)) );
            if ( !$stmt_last_lost_property = $police_dsr->prepare("SELECT SUM(property_details.lost_property) FROM property_details INNER JOIN dsr_entries ON property_details.dsr_id = dsr_entries.id WHERE property_details.office_id = ? AND (property_details.property_date BETWEEN ? AND ?) AND (dsr_entries.dsr_vidhan_ipc = ? OR dsr_entries.dsr_vidhan_ipc = ? OR dsr_entries.dsr_vidhan_ipc = ? OR dsr_entries.dsr_vidhan_ipc = ? OR dsr_entries.dsr_vidhan_ipc = ? OR dsr_entries.dsr_vidhan_ipc = ? OR dsr_entries.dsr_vidhan_ipc = ? OR dsr_entries.dsr_vidhan_ipc = ?)") ) 
            echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
            if ( !$stmt_last_lost_property->bind_param("issiiiiiiii", $branch_id, $last_year_datef, $last_year_date1, $a1, $a2, $a3, $a4, $a5, $a6, $a7, $a8) )
            echo "Binding Parameter Error: ($police_dsr->errno) $police_dsr->error";
            if ( !$stmt_last_lost_property->execute() ) 
            echo "Execute Error: ($stmt_last_lost_property->errno)  $stmt_last_lost_property->error";
            if ( !$stmt_last_lost_property->store_result() ) //Only for select with bind_result()
            echo "Storing Result Error: ($police_dsr->errno) $police_dsr->error";
            
            $stmt_last_lost_property->bind_result($cid_last_lost_property); 
            
            $stmt_last_lost_property->fetch();
            $s2[$j]=$cid_last_lost_property;
            ?>
        <span><?php if ($cid_last_lost_property>0){echo $cid_last_lost_property;}else{echo 0;}?></span>
        <?php
            $stmt_last_lost_property->close();
            ?>
        
        </td>
 
        <td class="गत वर्ष का आलौच्य पक्ष बरामद">
        <?php
                $police_dsr->select_db("ftcaaazc_dsr");
                $a1='3';
                $a2='5';
                ...
                $last_year_datef =   date('Y-m-d', strtotime('-1 year', strtotime($datef)) );
                $last_year_date1 = date('Y-m-d', strtotime('-1 year', strtotime($date1)) );
                if ( !$stmt_last_got_property = $police_dsr->prepare("SELECT SUM(property_details.got_property) FROM property_details INNER JOIN dsr_entries ON property_details.dsr_id = dsr_entries.id WHERE property_details.office_id = ? AND property_details.property_date BETWEEN ? AND ? AND (dsr_entries.dsr_vidhan_ipc = ? OR dsr_entries.dsr_vidhan_ipc = ? OR dsr_entries.dsr_vidhan_ipc = ? OR dsr_entries.dsr_vidhan_ipc = ? OR dsr_entries.dsr_vidhan_ipc = ? OR dsr_entries.dsr_vidhan_ipc = ? OR dsr_entries.dsr_vidhan_ipc = ? OR dsr_entries.dsr_vidhan_ipc = ?)") ) 
                echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
                if ( !$stmt_last_got_property->bind_param("issiiiiiiii",$branch_id, $last_year_datef, $last_year_date1, $a1, $a2, $a3, $a4, $a5, $a6, $a7, $a8) )
                echo "Binding Parameter Error: ($police_dsr->errno) $police_dsr->error";
                if ( !$stmt_last_got_property->execute() ) 
                echo "Execute Error: ($stmt_last_got_property->errno)  $stmt_last_got_property->error";
                if ( !$stmt_last_got_property->store_result() ) //Only for select with bind_result()
                echo "Storing Result Error: ($police_dsr->errno) $police_dsr->error";
                
                $stmt_last_got_property->bind_result($cid_got_property); 
               // echo $stmt_last_got_property->info;
                $stmt_last_got_property->fetch();
                $s_lost[$j]=$cid_got_property;
                ?>
            <span><?php if ($cid_got_property > 0){echo $cid_got_property ; }else{echo 0;}?></span>
            <?php
                $stmt_last_got_property->close();
                ?>
        </td>
        <td class="गत वर्ष का आलौच्य पक्ष बरामदगी का प्रतिशत">
        <?php
            if($cid_last_lost_property > 0)
            {	
            $percentage_last_year =($cid_got_property)*100/$cid_last_lost_property ;
            echo ROUND(ABS($percentage_last_year),2); echo "%";
            }
            else
            {	
            echo "0%";	
      ...
      <tr>
        ...
        <td colspan="3" class="आलौच्य पक्ष">
        <?php
        if($sum2 > 0)
        {	
        $percentage3 =($sum12)*100/$sum2 ;
        ...
        <td colspan="3" class="गत वर्ष का आलौच्य पक्ष" >0%</td>	
      </tr>
    </table>
    
    <br/><br/>
    <br/><br/>
	<table border="1" cellspacing="0" cellpadding="5" style="text-align:center" id="print" width="960px" class="जिला / रेंज वार संपत्ति  table table-striped table-bordered table-hover">
	<tr>
	<td>क्र.</td>

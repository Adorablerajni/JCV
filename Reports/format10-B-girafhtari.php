...
<title>Format-10-B-GIRAFHTARI WARRANT | File Tracking & Crime Analysis Application </title>
<!-- GLOBAL STYLES -->
...
<span style="float:right;margin-right:-25px"> <input type="hidden" id="datatodisplay" name="datatodisplay" style="" /> <input type="submit" value="Export to Excel" class="btn btn-warning"/>
 ...
 <div class="row">
    <div class="col-lg-12 label-primary" style="color:#FFF">
      <form action="format10-B-girafhtari.php" method="post" name="Search" enctype="multipart/form-data" id="popup-validation" role="form">
...
<div class="notice_all">
  ...
  <?php
   ...
   {
  ?>
<p align="left" style="float:left;margin-left:-30px;">Format-10-B-Girafhtari Warrant</p>
<p align="right"></p>
  <div class="mar10">
  <?php   
  ...
  ?>
    <p align="center"><span>गिरफ्तारी वारंट की तामीली के संबंध में गुण-दोष सहित टीप ।   दि. <?php if ($_POST['datef'] == ''){	echo '';} else{echo date('d-m-Y', strtotime($_POST['datef']));
 ...
 ?>
	...
	$sub_id ='No';
	$tamil_date_null = '';
	 if ( !$stmt2 = $police_summons->prepare("SELECT COUNT(id) FROM summon_list WHERE sp_id = ? AND warrant_type = ? and ((`samans_date` < ? and (`tamil_date` is null OR tamil_date =?)) OR (`samans_date` between ? and ?)) AND sub_id = ? AND sakshi_peshi_date > ?") )
    echo "Prepare Error: ($police_summons->errno) $police_summons->error";
    if ( !$stmt2->bind_param("isssssss",$branch_id, $warrant_type, $datef, $tamil_date_null, $datef, $datef, $sub_id, $datef) )
	...
	//$tamil_date ='NULL';
	$sub_id ='No';
	if ( !$stmt3 = $police_summons->prepare("SELECT COUNT(id) FROM summon_list WHERE sp_id = ? AND warrant_type = ? AND (samans_date between ? and ?) AND sub_id = ?") ) //AND (tamil_date is null OR tamil_date ='') 
    echo "Prepare Error: ($police_summons->errno) $police_summons->error";
	...
	$warrant_type ='गिरफ़्तारी वारंट';
	$sub_id ='No';
	if ( !$stmt4 = $police_summons->prepare("SELECT COUNT(id) FROM summon_list WHERE sp_id = ? AND warrant_type = ? AND  (tamil_date between ? AND ?) AND sub_id = ? and `samans_date` between ? and ?") ) 
    echo "Prepare Error: ($police_summons->errno) $police_summons->error";
    if ( !$stmt4->bind_param("issssss",$branch_id, $warrant_type, $datef, $date1, $sub_id, $datef, $date1) )
    echo "Binding Parameter Error: ($police_summons->errno) $police_summons->error";
    if ( !$stmt4->execute() ) 
	...
	<?php
		...
		//$aaropi_type92 = 'जनता';
	    //$date_now = CURDATE();
       if ( !$stmt5 = $police_summons->prepare("SELECT COUNT(id) FROM summon_list WHERE sp_id = ? AND sub_id = ? AND warrant_type = ?  and ((`sakshi_peshi_date` between ? and ? and (`tamil_date` = ? or `tamil_date` is null) and `samans_date` < ?) OR (`samans_date` < ? and `tamil_date` > ? and `sakshi_peshi_date` < ?))") ) 
        echo "Prepare Error: ($police_summons->errno) $police_summons->error";
        if ( !$stmt5->bind_param("isssssssss",$branch_id, $sub_id, $warrant_type92, $datef, $date1, $tamil_date92, $date1, $datef, $datef, $date1) )
        echo "Binding Parameter Error: ($police_summons->errno) $police_summons->error";
        if ( !$stmt5->execute() ) 
	...
	$warrant_type ='गिरफ़्तारी वारंट';
	$sub_id ='No';
	if ( !$stmt7 = $police_summons->prepare("SELECT COUNT(id) FROM summon_list WHERE sp_id = ? AND warrant_type = ? AND sub_id = ?  AND  (transfer_date between ? AND ?)") ) 
    echo "Prepare Error: ($police_summons->errno) $police_summons->error";
    if ( !$stmt7->bind_param("issss",$branch_id, $warrant_type, $sub_id, $datef, $date1) )
    echo "Binding Parameter Error: ($police_summons->errno) $police_summons->error";
    if ( !$stmt7->execute() ) 
    ...
    $stmt6->fetch();
	$ipc5[$j]=$cid5;?>
	<span><?php echo $shesh = $sum - ($cid3 + $cid4+ $cid6);?></span>
	<?php
	$stmt6->close();
	?>		
	</td>

...
class Diet_Shop_Post_Meta {
	...
	public function __construct() {
		...
		add_action( 'diet_shop_do_tags', array( $this, 'render_tags_list' ) );
		add_action( 'diet_shop_site_content_type', array( $this, 'render_meta_list' ), 5, 2 );
		add_action( 'diet_shop_meta_info', array( $this, 'render_meta_list' ), 10, 2 );
	...
	public function render_meta_list( $order, $wrp_class = '' ) {
		...
		$markup .= '<ul class="post-meta '.esc_attr( $wrp_class ).'">';
		foreach ( $order as $meta ) {
			switch ( $meta ) {
				case 'author':
					...
					$markup .= ' <span>'.esc_html__( 'By -','diet-shop' ).'</span>';
				...
				case 'date':
					...
					$markup .= esc_html__( 'Posted on ','diet-shop' );
					$markup .= $this->get_time_tags();
					$markup .= '</li>';
					break;
				case 'category':
					$markup .= '<li class="meta category">';
					$markup .= esc_html__( 'Posted in ','diet-shop' );
		...
		echo wp_kses( $markup, diet_shop_alowed_tags() );
	}
	...
	private function get_comments() {
		...
		if ( $comments_number == 0 ) {
			...
			$comments = sprintf( _n( '%s Comment', '%s Comments', $comments_number, 'diet-shop' ), $comments_number );
		}
	...
	public function render_tags_list() {
		...
		$html .= '<span>' . esc_html__( 'Tags', 'diet-shop' ) . ':</span>';
		foreach ( $tags as $tag ) {

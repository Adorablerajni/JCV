...
<span style="float:right;margin-right:-25px"> <input type="hidden" id="datatodisplay" name="datatodisplay" style="" /> <input type="submit" value="Export to Excel"/>
  ...
  <div class="row">
    <div class="col-lg-12">
      <form action="format11-testing.php" method="post" name="Search" enctype="multipart/form-data" id="popup-validation" role="form">
...
<?php 
    ...
    {  
	?>
  <li><a href="#" class="small" data-value="<?php echo $branch_name; //echo $get_sql_data['branch_name']?>, <?php echo $city; //echo $get_sql_data['city']?>" tabIndex="-1"><input type="checkbox" name="sp_office[]" value="<?php echo $id;?>"/>&nbsp;<?php echo $branch_name; //echo $get_sql_data['branch_name']?>, <?php echo $city; //echo $get_sql_data['city']?></a></li>
...
<?php
  ...
  //$data1=mysql_fetch_assoc($result1);
    //$chkArr = isset($_POST['sp_office']) ? $_POST['sp_office'] : array();
	//$chkArrCSV = implode(',',$chkArr);    

	/*
	$stmt1 = $police_tracking->prepare("SELECT id,branch_name,city FROM branch_tbl WHERE id IN (?)");
    $stmt1->bind_param("s", $chkArrCSV);
    $stmt1->execute();
    $stmt1->store_result();
    ...
    while ($stmt1->fetch())
	{
		*/
  ?>
    <p align="center"><span>अजा/अजजा की शिकायत पर पंजीबद्ध अपराधों की जानकारी जिनमें अजा/अजजा(अत्याचार-निवारण)अधिनियम 1989 की धारा लगायी गई होगी । <br /> दि <?php if ($_POST['datef'] == ''){	echo '';} else{echo date('d-m-Y', strtotime($_POST['datef']));
...
<table border="1" cellspacing="0" cellpadding="5" style="font-size:12px;line-height:20px;margin-left:-30px;text-align:center" id="print" width="960px">
  ...
  <tr>
	...
	<?php
	/*	
		//$lastnames =array($chkArrCSV);
	$arParams = array();
	
	//foreach($_POST['sp_office'] as $value) //recreate an array with parameters explicitly passing every parameter by reference
    //{
	echo $value;
	echo $ph;
	echo $ph1;
	$arParams[] = &$value;
	echo $params_1 = implode("'), ('", $arParams);
    ...
    echo $params = implode(',',$q);
	
	echo $lastnames[0];
    $w = array_fill(0,$count_params,'i');
    echo $params2 = implode(',',$w);
	$arParams = "'.mysql_real_escape_string($chkArrCSV).'";
	*/
	//$chkArr = $_POST['sp_office'];
	
	//$chkArr = isset($_POST['sp_office']) ? $_POST['sp_office'] : array();   
	//echo $chkArrCSV = implode(',',$chkArr);
	//echo $chkArrCSV = rtrim(str_repeat(''.$chkArr.',', count($_POST['sp_office'])), ','); 
    //$ph = rtrim(str_repeat('?,', count($_POST['sp_office'])), ',');
	//$ph1 = rtrim(str_repeat('i,', count($_POST['sp_office'])), ',');
	//echo $chkArrCSV;
	//echo $ph;
	//echo $ph1;
    if ($_POST['sp_office']!='')
	...
	$m=0;
	$len = count($_POST);
	$variable=$_POST['sp_office'];
    foreach ($variable as $variablename)
    {
		$sp= $_POST['sp_office'][$m];
		//echo "/";
 	$police_dsr->select_db("ftcaaazc_dsr");
	$stmt2 = $police_dsr->prepare("SELECT COUNT(id) FROM dsr_entries WHERE dsr_vidhan_ipc = ? AND sp_id = ? AND dsr_caste = ? AND (dsr_kaymi_date >= ? and dsr_kaymi_date <= ?)");
	$stmt2->bind_param("iisss", $j, $sp, $sc, $datef, $date1);//$chkArrCSV,'.$ph1.'
    $stmt2->execute();
    $stmt2->store_result();
	...
	?>
	<span><?php echo $sum ; //echo $cid1;?></span>
	<?php 
	}
	$stmt2->close();
	?>
    ...
    echo $chkArrCSV = implode(',',$chkArr);
 	$police_dsr->select_db("ftcaaazc_dsr");
	$stmt3 = $police_dsr->prepare("SELECT COUNT(id) FROM dsr_entries WHERE dsr_vidhan_ipc = ? AND sp_id IN ('.$chkArrCSV.') AND dsr_caste = ? AND (dsr_kaymi_date >= ? and dsr_kaymi_date <= ?)");
	$stmt3->bind_param("isss", $j, $st, $datef, $date1); //$branch_id, 
    $stmt3->execute();
    $stmt3->store_result();
    //if($stmt3->num_rows === 0) { echo "No Results"; }
	...
	$stmt4 = $police_dsr->prepare("SELECT COUNT(id) FROM dsr_entries WHERE dsr_vidhan_ipc = ? AND sp_id = ? AND dsr_caste = ? AND (dsr_kaymi_date >= ? and dsr_kaymi_date <= ?) AND c_status = ?");
	$stmt4->bind_param("iissss", $j, $branch_id, $sc, $datef, $date1,$vivechna);
    $stmt4->execute();
    $stmt4->store_result();
    //if($stmt3->num_rows === 0) { echo "No Results"; }
...
$ipc2[$i]=$cid3;	
	...
	<?php
 	$police_dsr->select_db("ftcaaazc_dsr");
	$vivechna = 'विवेचना में लंबित';
	$stmt5 = $police_dsr->prepare("SELECT COUNT(id) FROM dsr_entries WHERE dsr_vidhan_ipc = ? AND sp_id = ? AND dsr_caste = ? AND (dsr_kaymi_date >= ? and dsr_kaymi_date <= ?) AND c_status = ?");
	$stmt5->bind_param("iissss", $j, $branch_id, $st, $datef, $date1,$vivechna);
    $stmt5->execute();
    $stmt5->store_result();
    //if($stmt3->num_rows === 0) { echo "No Results"; }
...
$ipc3[$i]=$cid4;	
	...
	<?php
 	$police_dsr->select_db("ftcaaazc_dsr");
	$bandi = 'बंदी आरोपी';
	$stmt6 = $police_dsr->prepare("SELECT COUNT(id) FROM dsr_entries WHERE dsr_vidhan_ipc = ? AND sp_id = ? AND dsr_caste = ? AND (dsr_kaymi_date >= ? and dsr_kaymi_date <= ?) AND c_status = ?");
	$stmt6->bind_param("iissss", $j, $branch_id, $sc, $datef, $date1,$bandi);
    $stmt6->execute();
    $stmt6->store_result();
    //if($stmt3->num_rows === 0) { echo "No Results"; }
	...
	<?php
 	$police_dsr->select_db("ftcaaazc_dsr");
	$bandi = 'बंदी आरोपी';
	$stmt7 = $police_dsr->prepare("SELECT COUNT(id) FROM dsr_entries WHERE dsr_vidhan_ipc = ? AND sp_id = ? AND dsr_caste = ? AND (dsr_kaymi_date >= ? and dsr_kaymi_date <= ?) AND c_status = ?");
	$stmt7->bind_param("iissss", $j, $branch_id, $st, $datef, $date1,$bandi);
    $stmt7->execute();
    $stmt7->store_result();
    //if($stmt3->num_rows === 0) { echo "No Results"; }
...
$ipc5[$i]=$cid6;	
	...
	<?php
 	$police_dsr->select_db("ftcaaazc_dsr");
	$khatma = 'खात्मा';
	$stmt8 = $police_dsr->prepare("SELECT COUNT(id) FROM dsr_entries WHERE dsr_vidhan_ipc = ? AND sp_id = ? AND dsr_caste = ? AND (dsr_kaymi_date >= ? and dsr_kaymi_date <= ?) AND c_status = ?");
	$stmt8->bind_param("iissss", $j, $branch_id, $sc, $datef, $date1,$khatma);
    $stmt8->execute();
    $stmt8->store_result();
    //if($stmt3->num_rows === 0) { echo "No Results"; }
...
$ipc6[$i]=$cid7;	
	...
	<?php
 	$police_dsr->select_db("ftcaaazc_dsr");
	$khatma = 'खात्मा';
	$stmt9 = $police_dsr->prepare("SELECT COUNT(id) FROM dsr_entries WHERE dsr_vidhan_ipc = ? AND sp_id = ? AND dsr_caste = ? AND (dsr_kaymi_date >= ? and dsr_kaymi_date <= ?) AND c_status = ?");
	$stmt9->bind_param("iissss", $j, $branch_id, $st, $datef, $date1,$khatma);
    $stmt9->execute();
    $stmt9->store_result();
    //if($stmt3->num_rows === 0) { echo "No Results"; }
...
$ipc7[$i]=$cid8;	
	...
	<?php
 	$police_dsr->select_db("ftcaaazc_dsr");
	$kharji = 'खारची';
	$stmt10 = $police_dsr->prepare("SELECT COUNT(id) FROM dsr_entries WHERE dsr_vidhan_ipc = ? AND sp_id = ? AND dsr_caste = ? AND (dsr_kaymi_date >= ? and dsr_kaymi_date <= ?) AND c_status = ?");
	$stmt10->bind_param("iissss", $j, $branch_id, $sc, $datef, $date1,$kharji);
    $stmt10->execute();
    $stmt10->store_result();
    //if($stmt3->num_rows === 0) { echo "No Results"; }
...
$ipc8[$i]=$cid9;	
	...
	<?php
 	$police_dsr->select_db("ftcaaazc_dsr");
	$kharji = 'खारची';
	$stmt11 = $police_dsr->prepare("SELECT COUNT(id) FROM dsr_entries WHERE dsr_vidhan_ipc = ? AND sp_id = ? AND dsr_caste = ? AND (dsr_kaymi_date >= ? and dsr_kaymi_date <= ?) AND c_status = ?");
	$stmt11->bind_param("iissss", $j, $branch_id, $st, $datef, $date1,$kharji);
    $stmt11->execute();
    $stmt11->store_result();
    //if($stmt3->num_rows === 0) { echo "No Results"; }
...
$ipc9[$i]=$cid10;	
	...
	$stmt12 = $police_dsr->prepare("SELECT COUNT(id) FROM dsr_entries WHERE dsr_vidhan_ipc = ? AND sp_id = ? AND dsr_caste = ? AND (dsr_kaymi_date >= ? and dsr_kaymi_date <= ?) AND c_status = ?");
	$stmt12->bind_param("iissss", $j, $branch_id, $sc, $datef, $date1,$nyay);
    $stmt12->execute();
    $stmt12->store_result();
    //if($stmt3->num_rows === 0) { echo "No Results"; }
...
$ipc10[$i]=$cid11;	
	...
	<?php
 	$police_dsr->select_db("ftcaaazc_dsr");
	$nyay = 'न्यायालय में प्रस्तुत';
	$stmt13 = $police_dsr->prepare("SELECT COUNT(id) FROM dsr_entries WHERE dsr_vidhan_ipc = ? AND sp_id = ? AND dsr_caste = ? AND (dsr_kaymi_date >= ? and dsr_kaymi_date <= ?) AND c_status = ?");
	$stmt13->bind_param("iissss", $j, $branch_id, $st, $datef, $date1,$nyay);
    $stmt13->execute();
    $stmt13->store_result();
    //if($stmt3->num_rows === 0) { echo "No Results"; }
...
$ipc11[$i]=$cid12;	
	...
	<?php
 	$police_dsr->select_db("ftcaaazc_dsr");
	$dandit = 'सजा';
	$stmt14 = $police_dsr->prepare("SELECT COUNT(id) FROM dsr_entries WHERE dsr_vidhan_ipc = ? AND sp_id = ? AND dsr_caste = ? AND (dsr_kaymi_date >= ? and dsr_kaymi_date <= ?) AND c_decision = ?");
	$stmt14->bind_param("iissss", $j, $branch_id, $sc, $datef, $date1,$dandit);
    $stmt14->execute();
    $stmt14->store_result();
    //if($stmt3->num_rows === 0) { echo "No Results"; }
...
$ipc12[$i]=$cid13;	
	...
	<?php
 	$police_dsr->select_db("ftcaaazc_dsr");
	$dandit = 'सजा';
	$stmt15 = $police_dsr->prepare("SELECT COUNT(id) FROM dsr_entries WHERE dsr_vidhan_ipc = ? AND sp_id = ? AND dsr_caste = ? AND (dsr_kaymi_date >= ? and dsr_kaymi_date <= ?) AND c_decision = ?");
	$stmt15->bind_param("iissss", $j, $branch_id, $st, $datef, $date1,$dandit);
    $stmt15->execute();
    $stmt15->store_result();
    //if($stmt3->num_rows === 0) { echo "No Results"; }
...
$ipc13[$i]=$cid14;	
	...
	<?php
 	$police_dsr->select_db("ftcaaazc_dsr");
	$doshmukt = 'दोषमुक्त';
	$stmt16 = $police_dsr->prepare("SELECT COUNT(id) FROM dsr_entries WHERE dsr_vidhan_ipc = ? AND sp_id = ? AND dsr_caste = ? AND (dsr_kaymi_date >= ? and dsr_kaymi_date <= ?) AND c_decision = ?");
	$stmt16->bind_param("iissss", $j, $branch_id, $sc, $datef, $date1,$doshmukt);
    $stmt16->execute();
    $stmt16->store_result();
    //if($stmt3->num_rows === 0) { echo "No Results"; }
...
$ipc14[$i]=$cid15;	
	...
	<?php
 	$police_dsr->select_db("ftcaaazc_dsr");
	$doshmukt = 'दोषमुक्त';
	$stmt17 = $police_dsr->prepare("SELECT COUNT(id) FROM dsr_entries WHERE dsr_vidhan_ipc = ? AND sp_id = ? AND dsr_caste = ? AND (dsr_kaymi_date >= ? and dsr_kaymi_date <= ?) AND c_decision = ?");
	$stmt17->bind_param("iissss", $j, $branch_id, $st, $datef, $date1,$doshmukt);
    $stmt17->execute();
    $stmt17->store_result();
    //if($stmt3->num_rows === 0) { echo "No Results"; }
...
$ipc15[$i]=$cid16;	
	...
	$stmt18 = $police_dsr->prepare("SELECT COUNT(id) FROM dsr_entries WHERE dsr_vidhan_ipc = ? AND sp_id = ? AND dsr_caste = ? AND (dsr_kaymi_date >= ? and dsr_kaymi_date <= ?) AND c_status = ?");
	$stmt18->bind_param("iissss", $j, $branch_id, $sc, $datef, $date1,$nyay_lambit);
    $stmt18->execute();
    $stmt18->store_result();
    //if($stmt3->num_rows === 0) { echo "No Results"; }
...
$ipc16[$i]=$cid17;	
	...
	<?php
 	$police_dsr->select_db("ftcaaazc_dsr");
	$nyay_lambit = 'न्यायालय में लंबित';
	$stmt19 = $police_dsr->prepare("SELECT COUNT(id) FROM dsr_entries WHERE dsr_vidhan_ipc = ? AND sp_id = ? AND dsr_caste = ? AND (dsr_kaymi_date >= ? and dsr_kaymi_date <= ?) AND c_status = ?");
	$stmt19->bind_param("iissss", $j, $branch_id, $st, $datef, $date1,$nyay_lambit);
    $stmt19->execute();
    $stmt19->store_result();
    //if($stmt3->num_rows === 0) { echo "No Results"; }
...
$ipc17[$i]=$cid18;	
	...
	$stmt20 = $police_dsr->prepare("SELECT COUNT(id) FROM dsr_entries WHERE dsr_vidhan_ipc = ? AND sp_id = ? AND dsr_caste = ? AND (dsr_kaymi_date >= ? and dsr_kaymi_date <= ?) AND c_status = ?");
	$stmt20->bind_param("iissss", $j, $branch_id, $sc, $datef, $date1,$vivechna_dhin);
    $stmt20->execute();
    $stmt20->store_result();
    //if($stmt3->num_rows === 0) { echo "No Results"; }
...
$ipc18[$i]=$cid19;	
	...
	<?php
 	$police_dsr->select_db("ftcaaazc_dsr");
	$vivechna_dhin = 'विवेचनाधीन';
	$stmt21 = $police_dsr->prepare("SELECT COUNT(id) FROM dsr_entries WHERE dsr_vidhan_ipc = ? AND sp_id = ? AND dsr_caste = ? AND (dsr_kaymi_date >= ? and dsr_kaymi_date <= ?) AND c_status = ?");
	$stmt21->bind_param("iissss", $j, $branch_id, $st, $datef, $date1,$vivechna_dhin);
    $stmt21->execute();
    $stmt21->store_result();
    //if($stmt3->num_rows === 0) { echo "No Results"; }
...
$ipc19[$i]=$cid20;	
	...
	$stmt21->close();
	?>
	</td></tr><?php } //stmt1 while end here?>

...
<span style="float:right;margin-right:-25px"> <input type="hidden" id="datatodisplay" name="datatodisplay" style="" /> <input type="submit" value="Export to Excel" class="btn btn-warning"/>
 ...
 <div class="row">
    <div class="col-lg-12 label-primary" style="color:#FFF">
      <form action="format12-zone.php" method="post" name="Search" enctype="multipart/form-data" id="popup-validation" role="form">
  ...
  //$data1=mysql_fetch_assoc($result1);
	...
	{
  ?>
    <p align="center"><span>अजा/अजजा की शिकायत पर पंजीबद्ध अपराधों की जानकारी जिनमें अजा/अजजा(अत्याचार-निवारण)अधिनियम 1989 की धारा नहीं लगायी गई होगी । <br /> दि <?php if ($_POST['datef'] == ''){	echo '';} else{echo date('d-m-Y', strtotime($_POST['datef']));
...
<table border="1" cellspacing="0" cellpadding="5" style="text-align:center" id="print" width="960px" class="table table-striped table-bordered table-hover">
  ...
  <tr>
	...
	$stmt2 = $police_dsr->prepare("SELECT COUNT(id) FROM dsr_entries WHERE dsr_vidhan_ipc = ? AND dsr_caste = ? AND (dsr_kaymi_date >= ? and dsr_kaymi_date <= ?) AND (atyachar_adhiniyam = ? or atyachar_adhiniyam is null)");
	$stmt2->bind_param("issss", $j, $sc, $datef, $date1, $atyachar);
    $stmt2->execute();
    $stmt2->store_result();
	...
	<?php
 	$police_dsr->select_db("ftcaaazc_dsr");
	$stmt3 = $police_dsr->prepare("SELECT COUNT(id) FROM dsr_entries WHERE dsr_vidhan_ipc = ? AND dsr_caste = ? AND (dsr_kaymi_date >= ? and dsr_kaymi_date <= ?) AND (atyachar_adhiniyam = ? or atyachar_adhiniyam is null)");
	$stmt3->bind_param("issss", $j, $st, $datef, $date1, $atyachar);
    $stmt3->execute();
    $stmt3->store_result();
    //if($stmt3->num_rows === 0) { echo "No Results"; }
	...
	$stmt4 = $police_dsr->prepare("SELECT COUNT(id) FROM dsr_entries WHERE dsr_vidhan_ipc = ? AND dsr_caste = ? AND (dsr_kaymi_date >= ? and dsr_kaymi_date <= ?) AND c_status = ? AND (atyachar_adhiniyam = ? or atyachar_adhiniyam is null)");
	$stmt4->bind_param("isssss", $j, $sc, $datef, $date1,$vivechna, $atyachar);
    $stmt4->execute();
    $stmt4->store_result();
    //if($stmt3->num_rows === 0) { echo "No Results"; }
...
$ipc2[$i]=$cid3;	
	...
	<?php
 	$police_dsr->select_db("ftcaaazc_dsr");
	$vivechna = 'विवेचना में लंबित';
	$stmt5 = $police_dsr->prepare("SELECT COUNT(id) FROM dsr_entries WHERE dsr_vidhan_ipc = ? AND dsr_caste = ? AND (dsr_kaymi_date >= ? and dsr_kaymi_date <= ?) AND c_status = ? AND (atyachar_adhiniyam = ? or atyachar_adhiniyam is null)");
	$stmt5->bind_param("isssss", $j, $st, $datef, $date1,$vivechna, $atyachar);
    $stmt5->execute();
    $stmt5->store_result();
    //if($stmt3->num_rows === 0) { echo "No Results"; }
...
$ipc3[$i]=$cid4;	
	...
	<?php
 	$police_dsr->select_db("ftcaaazc_dsr");
	$bandi = 'बंदी आरोपी';
	$stmt6 = $police_dsr->prepare("SELECT COUNT(id) FROM dsr_entries WHERE dsr_vidhan_ipc = ? AND dsr_caste = ? AND (dsr_kaymi_date >= ? and dsr_kaymi_date <= ?) AND c_status = ? AND (atyachar_adhiniyam = ? or atyachar_adhiniyam is null)");
	$stmt6->bind_param("isssss", $j, $sc, $datef, $date1,$bandi, $atyachar);
    $stmt6->execute();
    $stmt6->store_result();
    //if($stmt3->num_rows === 0) { echo "No Results"; }
...
$ipc4[$i]=$cid5;	
	...
	<?php
 	$police_dsr->select_db("ftcaaazc_dsr");
	$bandi = 'बंदी आरोपी';
	$stmt7 = $police_dsr->prepare("SELECT COUNT(id) FROM dsr_entries WHERE dsr_vidhan_ipc = ? AND dsr_caste = ? AND (dsr_kaymi_date >= ? and dsr_kaymi_date <= ?) AND c_status = ? AND (atyachar_adhiniyam = ? or atyachar_adhiniyam is null)");
	$stmt7->bind_param("isssss", $j, $st, $datef, $date1,$bandi, $atyachar);
    $stmt7->execute();
    $stmt7->store_result();
    //if($stmt3->num_rows === 0) { echo "No Results"; }
...
$ipc5[$i]=$cid6;	
	...
	<?php
 	$police_dsr->select_db("ftcaaazc_dsr");
	$khatma = 'खात्मा';
	$stmt8 = $police_dsr->prepare("SELECT COUNT(id) FROM dsr_entries WHERE dsr_vidhan_ipc = ? AND dsr_caste = ? AND (dsr_kaymi_date >= ? and dsr_kaymi_date <= ?) AND c_status = ? AND (atyachar_adhiniyam = ? or atyachar_adhiniyam is null)");
	$stmt8->bind_param("isssss", $j, $sc, $datef, $date1,$khatma, $atyachar);
    $stmt8->execute();
    $stmt8->store_result();
    //if($stmt3->num_rows === 0) { echo "No Results"; }
...
$ipc6[$i]=$cid7;	
	...
	<?php
 	$police_dsr->select_db("ftcaaazc_dsr");
	$khatma = 'खात्मा';
	$stmt9 = $police_dsr->prepare("SELECT COUNT(id) FROM dsr_entries WHERE dsr_vidhan_ipc = ? AND dsr_caste = ? AND (dsr_kaymi_date >= ? and dsr_kaymi_date <= ?) AND c_status = ? AND (atyachar_adhiniyam = ? or atyachar_adhiniyam is null)");
	$stmt9->bind_param("isssss", $j, $st, $datef, $date1,$khatma, $atyachar);
    $stmt9->execute();
    $stmt9->store_result();
    //if($stmt3->num_rows === 0) { echo "No Results"; }
...
$ipc7[$i]=$cid8;	
	...
	<?php
 	$police_dsr->select_db("ftcaaazc_dsr");
	$kharji = 'खारची';
	$stmt10 = $police_dsr->prepare("SELECT COUNT(id) FROM dsr_entries WHERE dsr_vidhan_ipc = ? AND dsr_caste = ? AND (dsr_kaymi_date >= ? and dsr_kaymi_date <= ?) AND c_status = ? AND (atyachar_adhiniyam = ? or atyachar_adhiniyam is null)");
	$stmt10->bind_param("isssss", $j, $sc, $datef, $date1,$kharji, $atyachar);
    $stmt10->execute();
    $stmt10->store_result();
    //if($stmt3->num_rows === 0) { echo "No Results"; }
...
$ipc8[$i]=$cid9;	
	...
	<?php
 	$police_dsr->select_db("ftcaaazc_dsr");
	$kharji = 'खारची';
	$stmt11 = $police_dsr->prepare("SELECT COUNT(id) FROM dsr_entries WHERE dsr_vidhan_ipc = ? AND dsr_caste = ? AND (dsr_kaymi_date >= ? and dsr_kaymi_date <= ?) AND c_status = ? AND (atyachar_adhiniyam = ? or atyachar_adhiniyam is null)");
	$stmt11->bind_param("isssss", $j, $st, $datef, $date1,$kharji, $atyachar);
    $stmt11->execute();
    $stmt11->store_result();
    //if($stmt3->num_rows === 0) { echo "No Results"; }
...
$ipc9[$i]=$cid10;	
	...
	$stmt12 = $police_dsr->prepare("SELECT COUNT(id) FROM dsr_entries WHERE dsr_vidhan_ipc = ? AND dsr_caste = ? AND (dsr_kaymi_date >= ? and dsr_kaymi_date <= ?) AND c_status = ? AND (atyachar_adhiniyam = ? or atyachar_adhiniyam is null)");
	$stmt12->bind_param("isssss", $j, $sc, $datef, $date1,$nyay, $atyachar);
    $stmt12->execute();
    $stmt12->store_result();
    //if($stmt3->num_rows === 0) { echo "No Results"; }
...
$ipc10[$i]=$cid11;	
	...
	<?php
 	$police_dsr->select_db("ftcaaazc_dsr");
	$nyay = 'न्यायालय में प्रस्तुत';
	$stmt13 = $police_dsr->prepare("SELECT COUNT(id) FROM dsr_entries WHERE dsr_vidhan_ipc = ? AND dsr_caste = ? AND (dsr_kaymi_date >= ? and dsr_kaymi_date <= ?) AND c_status = ? AND (atyachar_adhiniyam = ? or atyachar_adhiniyam is null)");
	$stmt13->bind_param("isssss", $j, $st, $datef, $date1,$nyay, $atyachar);
    $stmt13->execute();
    $stmt13->store_result();
    //if($stmt3->num_rows === 0) { echo "No Results"; }
...
$ipc11[$i]=$cid12;	
	...
	<?php
 	$police_dsr->select_db("ftcaaazc_dsr");
	$dandit = 'सजा';
	$stmt14 = $police_dsr->prepare("SELECT COUNT(id) FROM dsr_entries WHERE dsr_vidhan_ipc = ? AND dsr_caste = ? AND (dsr_kaymi_date >= ? and dsr_kaymi_date <= ?) AND c_decision = ? AND (atyachar_adhiniyam = ? or atyachar_adhiniyam is null)");
	$stmt14->bind_param("isssss", $j, $sc, $datef, $date1,$dandit, $atyachar);
    $stmt14->execute();
    $stmt14->store_result();
    //if($stmt3->num_rows === 0) { echo "No Results"; }
...
$ipc12[$i]=$cid13;	
	...
	<?php
 	$police_dsr->select_db("ftcaaazc_dsr");
	$dandit = 'सजा';
	$stmt15 = $police_dsr->prepare("SELECT COUNT(id) FROM dsr_entries WHERE dsr_vidhan_ipc = ? AND dsr_caste = ? AND (dsr_kaymi_date >= ? and dsr_kaymi_date <= ?) AND c_decision = ? AND (atyachar_adhiniyam = ? or atyachar_adhiniyam is null)");
	$stmt15->bind_param("isssss", $j, $st, $datef, $date1,$dandit, $atyachar);
    $stmt15->execute();
    $stmt15->store_result();
    //if($stmt3->num_rows === 0) { echo "No Results"; }
...
$ipc13[$i]=$cid14;	
	...
	<?php
 	$police_dsr->select_db("ftcaaazc_dsr");
	$doshmukt = 'दोषमुक्त';
	$stmt16 = $police_dsr->prepare("SELECT COUNT(id) FROM dsr_entries WHERE dsr_vidhan_ipc = ? AND dsr_caste = ? AND (dsr_kaymi_date >= ? and dsr_kaymi_date <= ?) AND c_decision = ? AND (atyachar_adhiniyam = ? or atyachar_adhiniyam is null)");
	$stmt16->bind_param("isssss", $j, $sc, $datef, $date1,$doshmukt, $atyachar);
    $stmt16->execute();
    $stmt16->store_result();
    //if($stmt3->num_rows === 0) { echo "No Results"; }
...
$ipc14[$i]=$cid15;	
	...
	<?php
 	$police_dsr->select_db("ftcaaazc_dsr");
	$doshmukt = 'दोषमुक्त';
	$stmt17 = $police_dsr->prepare("SELECT COUNT(id) FROM dsr_entries WHERE dsr_vidhan_ipc = ? AND dsr_caste = ? AND (dsr_kaymi_date >= ? and dsr_kaymi_date <= ?) AND c_decision = ? AND (atyachar_adhiniyam = ? or atyachar_adhiniyam is null)");
	$stmt17->bind_param("isssss", $j, $st, $datef, $date1,$doshmukt, $atyachar);
    $stmt17->execute();
    $stmt17->store_result();
    //if($stmt3->num_rows === 0) { echo "No Results"; }
...
$ipc15[$i]=$cid16;	
	...
	$stmt18 = $police_dsr->prepare("SELECT COUNT(id) FROM dsr_entries WHERE dsr_vidhan_ipc = ? AND dsr_caste = ? AND (dsr_kaymi_date >= ? and dsr_kaymi_date <= ?) AND c_status = ? AND (atyachar_adhiniyam = ? or atyachar_adhiniyam is null)");
	$stmt18->bind_param("isssss", $j, $sc, $datef, $date1,$nyay_lambit, $atyachar);
    $stmt18->execute();
    $stmt18->store_result();
    //if($stmt3->num_rows === 0) { echo "No Results"; }
...
$ipc16[$i]=$cid17;	
	...
	<?php
 	$police_dsr->select_db("ftcaaazc_dsr");
	$nyay_lambit = 'न्यायालय में लंबित';
	$stmt19 = $police_dsr->prepare("SELECT COUNT(id) FROM dsr_entries WHERE dsr_vidhan_ipc = ? AND dsr_caste = ? AND (dsr_kaymi_date >= ? and dsr_kaymi_date <= ?) AND c_status = ? AND (atyachar_adhiniyam = ? or atyachar_adhiniyam is null)");
	$stmt19->bind_param("isssss", $j, $st, $datef, $date1,$nyay_lambit, $atyachar);
    $stmt19->execute();
    $stmt19->store_result();
    //if($stmt3->num_rows === 0) { echo "No Results"; }
...
$ipc17[$i]=$cid18;	
	...
	$stmt20 = $police_dsr->prepare("SELECT COUNT(id) FROM dsr_entries WHERE dsr_vidhan_ipc = ? AND dsr_caste = ? AND (dsr_kaymi_date >= ? and dsr_kaymi_date <= ?) AND c_status = ? AND (atyachar_adhiniyam = ? or atyachar_adhiniyam is null)");
	$stmt20->bind_param("isssss", $j, $sc, $datef, $date1,$vivechna_dhin, $atyachar);
    $stmt20->execute();
    $stmt20->store_result();
    //if($stmt3->num_rows === 0) { echo "No Results"; }
...
$ipc18[$i]=$cid19;	
	...
	<?php
 	$police_dsr->select_db("ftcaaazc_dsr");
	$vivechna_dhin = 'विवेचनाधीन';
	$stmt21 = $police_dsr->prepare("SELECT COUNT(id) FROM dsr_entries WHERE dsr_vidhan_ipc = ? AND dsr_caste = ? AND (dsr_kaymi_date >= ? and dsr_kaymi_date <= ?) AND c_status = ? AND (atyachar_adhiniyam = ? or atyachar_adhiniyam is null)");
	$stmt21->bind_param("isssss", $j, $st, $datef, $date1,$vivechna_dhin, $atyachar);
    $stmt21->execute();
    $stmt21->store_result();
    //if($stmt3->num_rows === 0) { echo "No Results"; }

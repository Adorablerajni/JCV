...
$ipc6[$j]=$cid7;	
	...
	$four_wheeler = '70';
	//$theft1 = '9';
	if ( !$stmt9 = $police_dsr->prepare("SELECT COUNT(id) FROM dsr_entries WHERE sp_id = ? AND dsr_kaymi_date BETWEEN ? AND ? AND (dsr_vidhan_ipc = ? or dsr_vidhan_ipc = ? or dsr_vidhan_ipc = ?)") ) 
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
...
$ipc11[$j]=$cid12;	
	...
	$police_dsr->select_db("ftcaaazc_dsr");	
	$gum = '102';
	if ( !$stmt14 = $police_dsr->prepare("SELECT COUNT(id) FROM dsr_entries where sp_id = ? and dsr_vidhan_ipc is not NULL and dsr_vidhan_ipc != '' AND dsr_vidhan_ipc != ? AND dsr_kaymi_date BETWEEN ? AND ?") ) //
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
...
$ipc12[$j]=$cid13;
	...
	if ( !$stmt15= $police_dsr->prepare("SELECT COUNT(id) FROM dsr_entries WHERE sp_id = ? AND dsr_kaymi_date BETWEEN ? AND ? AND (dsr_vidhan_ipc = ? OR dsr_vidhan_ipc = ?) AND dsr_kdnp_rsn = ?") ) 
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
	...
	if ( !$stmt16= $police_dsr->prepare("SELECT COUNT(id) FROM dsr_entries WHERE sp_id = ? AND dsr_kaymi_date BETWEEN ? AND ? AND (dsr_vidhan_ipc = ? OR dsr_vidhan_ipc = ?) AND dsr_kdnp_rsn = ?") ) 
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
...
$ipc15[$j]=$cid15;	
	...
	$kdnp ='13';
	if ( !$stmt17= $police_dsr->prepare("SELECT COUNT(id) FROM dsr_entries WHERE sp_id = ? AND dsr_kaymi_date BETWEEN ? AND ? AND (dsr_vidhan_ipc = ? OR dsr_vidhan_ipc = ?) AND dsr_gender = ?") ) 
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
...
$ipc16[$j]=$cid16;	
	...
	$kidnapping_ranson = 'फिरौती के लिये';
	$kdnp ='13';
	if ( !$stmt18= $police_dsr->prepare("SELECT COUNT(id) FROM dsr_entries WHERE sp_id = ? AND (dsr_kaymi_date BETWEEN ? AND ?) AND (dsr_vidhan_ipc = ? OR dsr_vidhan_ipc = ?) AND (dsr_kdnp_rsn != ? OR dsr_kdnp_rsn != ?) and dsr_gender !=?") ) 
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
...
$ipc17[$j]=$cid17;	
	...
	<?php
 	$police_dsr->select_db("ftcaaazc_dsr");
	if ( !$stmt20= $police_dsr->prepare("SELECT SUM(dsr_theft_amount) FROM dsr_entries WHERE sp_id = ? AND dsr_kaymi_date BETWEEN ? AND ? ") ) 
    echo "Prepare Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt20->bind_param("iss", $idcheck, $datef, $date1) )
    echo "Binding Parameter Error: ($police_dsr->errno) $police_dsr->error";
    if ( !$stmt20->execute() ) 

...
<p align="right"></p>
 ...
 ?>
	...
	<?php
		...
		//$aaropi_type92 = 'जनता';
	    //$date_now = CURDATE();
        if ( !$stmt5 = $police_summons->prepare("SELECT COUNT(id) FROM summon_list WHERE sp_id = ? AND sub_id = ? AND warrant_type = ? AND (tamil_date = ? or tamil_date is null) AND sakshi_peshi_date < CURDATE() ") ) 
        echo "Prepare Error: ($police_summons->errno) $police_summons->error";
        if ( !$stmt5->bind_param("isss",$branch_id, $sub_id, $warrant_type92, $tamil_date92) )
        echo "Binding Parameter Error: ($police_summons->errno) $police_summons->error";
        if ( !$stmt5->execute() ) 
    ...
    $stmt6->fetch();
	$ipc5[$j]=$cid5;?>
	<span><?php echo $shesh = $sum - ($cid3 + $cid4);?></span>
	<?php
	$stmt6->close();
	?>		
	</td>
